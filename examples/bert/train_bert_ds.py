...
from megatron_deepspeed_tpu.debugger.module_debugger import print_log, register_hook, save_model_params, combine_npz
...
def create_model(num_layers: int, num_heads: int, ff_dim: int, h_dim: int,
    ...
    roberta_model.apply(register_hook)
...
def train(
    ...
    model, _, _, _ = deepspeed.initialize(model=model,
                                          ...
                                          config=ds_config)
    if device.type == "tpu":
        model.lm_head.decoder.weight = model.encoder.embeddings.word_embeddings.weight
    ...
    for step, batch in enumerate(data_iterator, start=start_step):
        ...
        # Forward pass
        loss = model(**batch)
        print_log("Loss: {0:.4f}".format(loss.item()))
        # Backward pass
        model.backward(loss)
        # Optimizer Step
        model.step()
        save_model_params(model, step)

...
void GathervOptions::setOutput(
  ...
  this->setElementSize(elementSize);
  SOPHON_ENFORCE_EQ(elementsPerRank.size(), context->size);
  this->elementsPerRank = std::move(elementsPerRank);
  SOPHON_ENFORCE_EQ(totalElements * elementSize, buf->size);
  ...
  if (context->rank == opts.root) {
    ...
    for (int i = 0; i < context->size; i++) {
      ...
      if (i != context->rank) {
        ...
        // Local memory copy
        SOPHON_ENFORCE_EQ(copyLength, in->size);
        if (copyLength > 0) {
          memcpy(
    ...
    SOPHON_ENFORCE_GE(in->size, sendLength);

...
void Pair::connect(const std::vector<char>& bytes) {
  ...
  SOPHON_ENFORCE_EQ(state_, INITIALIZING);
  state_ = CONNECTING;
...
void Pair::connectCallback(std::shared_ptr<Socket> socket, Error error) {
  std::lock_guard<std::mutex> lock(m_);
  if (error) {
    signalException(SOPHON_ERROR_MSG(error.what()));
    return;
  }
...
void Pair::setSync(bool sync, bool busyPoll) {
  ...
  if (!sync) {
    SOPHON_THROW_INVALID_OPERATION_EXCEPTION("Can only switch to sync mode");
  }
  ...
  if (state_ == CLOSED) {
    signalAndThrowException(
        SOPHON_ERROR_MSG("Socket unexpectedly closed ", peer_.str()));
  }
    ...
    if (rv == -1) {
      if (errno == EAGAIN) {
        if (sync_) {
          ...
          signalException(SOPHON_ERROR_MSG("Write timeout ", peer_.str()));
        } else {
      ...
      // Unexpected error
      signalException(
          SOPHON_ERROR_MSG("writev ", peer_.str(), ": ", strerror(errno)));
      return false;
    }
...
ssize_t Pair::prepareRead(
  ...
  if (opcode == Op::SEND_BUFFER) {
    ...
    SOPHON_ENFORCE_LE(op.preamble.roffset + op.preamble.length, op.buf->size_);
    return iov.iov_len;
  }
  ...
  if (opcode == Op::SEND_UNBOUND_BUFFER) {
    if (!op.ubuf) {
      ...
      SOPHON_ENFORCE(it != localPendingRecv_.end());
      std::deque<UnboundBufferOp>& queue = it->second;
      SOPHON_ENFORCE(!queue.empty());
    ...
    // Bytes read must be in bounds for target buffer
    SOPHON_ENFORCE_LE(op.preamble.length, op.nbytes);
    return iov.iov_len;
  }
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (rv == -1) {
        ...
        if (errno == EAGAIN) {
          if (sync_) {
            ...
            if (busyPoll_ && !hasTimedOut()) {
              ...
              // indicates timeout
              signalException(SOPHON_ERROR_MSG("Read timeout ", peer_.str()));
            }
          } else {
        ...
        // Unexpected error
        signalException(
            SOPHON_ERROR_MSG("Read error ", peer_.str(), ": ", strerror(errno)));
        return false;
      }
      break;
    }
...
void Pair::handleEvents(int events) {
  ...
  SOPHON_ENFORCE(false, "Unexpected state: ", state_);
}
...
void Pair::verifyConnected() {
  ...
  if (state_ == CLOSED) {
    signalAndThrowException(SOPHON_ERROR_MSG("Socket closed ", peer_.str()));
  }
}
...
std::unique_ptr<::sophon::transport::Buffer> Pair::createSendBuffer(
    int slot,
    void* ptr,
    size_t size) {
  auto buffer = new Buffer(this, slot, ptr, size);
  return std::unique_ptr<::sophon::transport::Buffer>(buffer);
}

std::unique_ptr<::sophon::transport::Buffer> Pair::createRecvBuffer(
    int slot,
    void* ptr,
    size_t size) {
  auto buffer = new Buffer(this, slot, ptr, size);
...
void Pair::send(
  ...
  if (nbytes > 0) {
    SOPHON_ENFORCE_LE(offset, tbuf->size);
    SOPHON_ENFORCE_LE(nbytes, tbuf->size - offset);
  }
...
void Pair::signalException(const std::string& msg) {
  signalException(std::make_exception_ptr(::sophon::IoException(msg)));
}
...
void Pair::signalAndThrowException(const std::string& msg) {
  signalAndThrowException(std::make_exception_ptr(::sophon::IoException(msg)));
}

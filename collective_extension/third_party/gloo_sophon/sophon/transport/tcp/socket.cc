...
#include <sophon/transport/tcp/socket.h>
...
std::shared_ptr<Socket> Socket::createForFamily(sa_family_t ai_family) {
  ...
  SOPHON_ENFORCE_NE(rv, -1, "socket: ", strerror(errno));
...
void Socket::reuseAddr(bool on) {
  ...
  SOPHON_ENFORCE_NE(rv, -1, "setsockopt: ", strerror(errno));
}
...
void Socket::block(bool on) {
  ...
  SOPHON_ENFORCE_NE(rv, -1, "fcntl: ", strerror(errno));
  if (!on) {
    // Set O_NONBLOCK
    rv |= O_NONBLOCK;
  } else {
...
void Socket::bind(const sockaddr_storage& ss) {
  ...
  SOPHON_ENFORCE(false, "Unknown address family: ", ss.ss_family);
}

void Socket::bind(const struct sockaddr* addr, socklen_t addrlen) {
  auto rv = ::bind(fd_, addr, addrlen);
  SOPHON_ENFORCE_NE(rv, -1, "bind: ", strerror(errno));
}

void Socket::listen(int backlog) {
  auto rv = ::listen(fd_, backlog);
  SOPHON_ENFORCE_NE(rv, -1, "listen: ", strerror(errno));
}
  ...
  for (;;) {
    ...
    if (rv == -1) {
      ...
      if (errno != EINPROGRESS) {
        SOPHON_ENFORCE_NE(rv, -1, "connect: ", strerror(errno));
      }
    }
    break;
  }

...
#endif
  SOPHON_ENFORCE_EQ(buf.data(), ret, "realpath: ", strerror(errno));
...
void FileStore::set(const std::string& key, const std::vector<char>& data) {
...

  {
    ...
    SOPHON_ENFORCE(!ifs.is_open(), "File already exists: ", path);
  }
  ...
  SOPHON_ENFORCE_EQ(rv, 0, "rename: ", strerror(errno));
}
...
bool FileStore::check(const std::vector<std::string>& keys) {
  ...
  for (const auto& path : paths) {
    ...
    if (fd == -1) {
      ...
      SOPHON_ENFORCE_EQ(errno, ENOENT);

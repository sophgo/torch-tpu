...
#define SOPHON_STORE_HAS_STORE_V2 1
...
class Store: public IStore {
 public:
  ...
  virtual std::vector<std::vector<char>> multi_get(const std::vector<std::string>& /*keys*/) {
    SOPHON_THROW_INVALID_OPERATION_EXCEPTION("this store doesn't support multi_get");
  }

  virtual void multi_set(const std::vector<std::string>& /*keys*/, const std::vector<std::vector<char>>& /*values*/) {
    SOPHON_THROW_INVALID_OPERATION_EXCEPTION("this store doesn't support multi_set");
  }

  virtual void append(const std::string& key, const std::vector<char>& /*data*/) {
    SOPHON_THROW_INVALID_OPERATION_EXCEPTION("this store doesn't support append");
  }

  virtual int64_t add(const std::string& key, int64_t value) {
    SOPHON_THROW_INVALID_OPERATION_EXCEPTION("this store doesn't support add");
  }

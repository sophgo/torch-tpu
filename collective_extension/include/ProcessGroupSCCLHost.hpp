...
// ProcessGroupSCCLHost implements Gloo bindings for c10d.
//
...
class TORCH_API ProcessGroupSCCLHost : public ProcessGroup {
 public:
  ...
  class TORCH_API AsyncWork : public Work {
   ...
   protected:
    friend class ProcessGroupSCCLHost;
    virtual void finishWorkSCCLHost() = 0;
  ...
  const std::string getBackendName() const override {
    return std::string(SCCL_HOST_BACKEND_NAME);
  }
  ...
  explicit ProcessGroupSCCLHost(
      const c10::intrusive_ptr<Store>& store,
      int rank,
      int size,
      c10::intrusive_ptr<Options> options = Options::create());

  virtual ~ProcessGroupSCCLHost();
...
public:
  ...
  static void ProcessGroupSCCLHostConstructor() __attribute__((constructor)) {
    py::object module = py::module::import("torch.distributed");
    py::object register_backend =
        module.attr("Backend").attr("register_backend");
    register_backend("SCCLHOST", py::cpp_function(createProcessGroupSCCLHost));
  }

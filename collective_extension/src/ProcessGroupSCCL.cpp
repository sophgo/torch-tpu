...
// static
void ProcessGroupSCCL::AsyncWork::execute(c10::intrusive_ptr<AsyncWork> work) {
...
c10::intrusive_ptr<c10::ivalue::Future> ProcessGroupSCCL::AsyncWork::
    getFuture() {
  return future_;
}
...
inline void ProcessGroupSCCL::AsyncWork::recordAsyncWorkProfilingInfo(
...
ProcessGroupSCCL::AsyncWork::AsyncWork(
...
void ProcessGroupSCCL::AsyncWork::finishWorkGlooError(std::exception_ptr eptr) {
  future_->setError(eptr);
  finish(eptr);
}

void ProcessGroupSCCL::AsyncWork::finishWorkGloo() {
  returnFutureWithOutput(future_, outputTensors_);
  finish();
}


ProcessGroupSCCL::SendWork::SendWork(
    at::Tensor& tensor,
...
bool ProcessGroupSCCL::SendWork::wait(std::chrono::milliseconds timeout) {
...
void ProcessGroupSCCL::SendWork::abort() {
  buffer_->abortWaitSend();
}

ProcessGroupSCCL::RecvWork::RecvWork(
    at::Tensor& tensor,
    std::unique_ptr<::gloo::transport::UnboundBuffer> buffer,
    const char* profilingTitle)
    : Work(
...
int ProcessGroupSCCL::RecvWork::sourceRank() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return srcRank_;
}

bool ProcessGroupSCCL::RecvWork::wait(std::chrono::milliseconds timeout) {
...
ProcessGroupSCCL::Options::Options(std::chrono::milliseconds timeout)
    : ProcessGroup::Options(SCCL_BACKEND_NAME, timeout), threads(2) {}
...
std::shared_ptr<::gloo::transport::Device> ProcessGroupSCCL::
...
ProcessGroupSCCL::ProcessGroupSCCL(
  ...
  for (const auto i : c10::irange(threads_.size())) {
    threads_[i] = std::thread(&ProcessGroupSCCL::runLoop, this, i);
  }
...
uint32_t ProcessGroupSCCL::nextTag() {
  return collectiveCounter_++;
}

std::shared_ptr<::gloo::Context> ProcessGroupSCCL::getContext(uint32_t tag) {
  return contexts_[tag % contexts_.size()];
}

void ProcessGroupSCCL::runLoop(int workerIndex) {
  std::unique_lock<std::mutex> lock(workMutex_);
...
void ProcessGroupSCCL::enqueue(c10::intrusive_ptr<AsyncWork> work) {
  std::unique_lock<std::mutex> lock(workMutex_);
...
class AsyncBroadcastWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncBroadcastWork(
      ...
      int rootTensor,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({inputs}, "gloo:broadcast", inputs),
        context(context),
        inputs(inputs),
        rootRank(rootRank),
...
class AsyncAllreduceWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncAllreduceWork(
      const std::shared_ptr<gloo::Context>& context,
      std::vector<at::Tensor>& inputs,
      ReduceOp reduceOp,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({inputs}, "gloo:all_reduce", inputs),
        context(context),
        inputs(inputs),
        reduceOp(reduceOp),
        tag(tag) {}
...
class AsyncAllreduceTPUWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncAllreduceTPUWork(
      ...
      ReduceOp reduceOp,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({inputs_cpu}, "gloo:all_reduce", inputs_cpu),
        context(context),
        inputs(inputs),
...
class AsyncSparseAllreduceWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncSparseAllreduceWork(
      const std::shared_ptr<gloo::Context>& context,
      std::vector<at::Tensor>& inputs,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({inputs}, "gloo:sparse_all_reduce", inputs),
        context(context),
        inputs(inputs),
        tag(tag) {}
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::allreduce(
    std::vector<at::Tensor>& inputs,
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::allreduce_coalesced(
    std::vector<at::Tensor>& tensors,
    const AllreduceCoalescedOptions& opts) {
  static auto invalidArgument = [](const std::string& msg) {
    TORCH_CHECK(false, "ProcessGroupSCCL::allreduce_coalesced: " + msg);
  };
...
class AsyncReduceWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncReduceWork(
      const std::shared_ptr<gloo::Context>& context,
      std::vector<at::Tensor>& inputs,
      ...
      ReduceOp reduceOp,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({inputs}, "gloo:reduce", inputs),
        context(context),
        inputs(inputs),
        rootRank(rootRank),
        rootTensor(rootTensor),
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::reduce(
    std::vector<at::Tensor>& inputs,
...
class AsyncAllgatherWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncAllgatherWork(
      const std::shared_ptr<gloo::Context>& context,
      ...
      std::vector<at::Tensor>& inputs,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork(outputs, "gloo:all_gather", inputs),
        context(context),
        outputs(outputs),
        inputs(inputs),
        tag(tag) {}
...
// Note: current CUDA implementation holds the assumption that the
// tensors in the nested output tensor vectors are on the same device.
c10::intrusive_ptr<Work> ProcessGroupSCCL::allgather(
    std::vector<std::vector<at::Tensor>>& outputs,
    std::vector<at::Tensor>& inputs,
...
class AsyncAllgatherCoalescedWork : public ProcessGroupSCCL::AsyncWork {
 public:
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::allgather_coalesced(
    std::vector<std::vector<at::Tensor>>& output_lists,
    std::vector<at::Tensor>& input_list,
    const AllgatherOptions& /* unused */) {
  static auto invalidArgument = [](const std::string& msg) {
    TORCH_CHECK(false, "ProcessGroupSCCL::allgather_coalesced: " + msg);
  };
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::_allgather_base(
    at::Tensor& /*unused */,
    at::Tensor& /*unused */,
    const AllgatherOptions& /*unused */) {
...
class AsyncGatherWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncGatherWork(
      const std::shared_ptr<gloo::Context>& context,
      std::vector<std::vector<at::Tensor>>& outputs,
      ...
      int root,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork(outputs, "gloo:gather", inputs),
        context(context),
        outputs(outputs),
        inputs(inputs),
        root(root),
...
class AsyncScatterWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncScatterWork(
      const std::shared_ptr<gloo::Context>& context,
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::reduce_scatter(
    std::vector<at::Tensor>& outputs,
    std::vector<std::vector<at::Tensor>>& inputs,
...
class AsyncAlltoallWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncAlltoallWork(
      const std::shared_ptr<gloo::Context>& context,
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::send(
    std::vector<at::Tensor>& tensors,
    int dstRank,
    int tag) {
...
class AsyncBarrierWork : public ProcessGroupSCCL::AsyncWork {
 public:
  AsyncBarrierWork(
      const std::shared_ptr<gloo::Context>& context,
      std::vector<c10::weak_intrusive_ptr<AsyncWork>> priorWork,
      uint32_t tag)
      : ProcessGroupSCCL::AsyncWork({}, "gloo:barrier", c10::nullopt),
        context(context),
...
c10::intrusive_ptr<Work> ProcessGroupSCCL::barrier(const BarrierOptions& opts) {
  std::vector<c10::weak_intrusive_ptr<AsyncWork>> priorWork;
...
void ProcessGroupSCCL::monitoredBarrier(
...
c10::intrusive_ptr<ProcessGroup> ProcessGroupSCCL::createProcessGroupSCCL(

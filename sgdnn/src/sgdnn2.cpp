                            ...
                            bool non_blocking)
{
  ...
  api.other_dtype = sgdnnTPUKernelDType ( other.dtype );
  api.output_dtype = sgdnnTPUKernelDType ( output.dtype );
  SAFE_CALL ( sgdnnTPUKernelLaunch ( stream, "tpu_kernel_api_pow_bcast_multi_core", &api, sizeof ( api ), non_blocking) );
  return tpuRtSuccess;
}
                      ...
                      bool non_blocking )
{
  ...
  api.dtype = sgdnnTPUKernelDType ( self.dtype );
  api.out_is_int = (out.dtype == SGDNN_DTYPE_INT32);
  SAFE_CALL ( sgdnnTPUKernelLaunch ( stream, "tpu_kernel_api_c_pow_multi_core", &api, sizeof ( api ), non_blocking) );
  return tpuRtSuccess;
}

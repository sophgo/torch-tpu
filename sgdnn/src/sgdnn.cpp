...
bm_status_t sgdnnReciprocal ( bm_handle_t handle,
  ...
  api.input_global_addr = input.addr;
  api.output_global_addr = output.addr;
  SAFE_CALL ( sgdnnTPUKernelLaunch ( handle, "tpu_kernel_api_reciprocal", &api, sizeof( api ) ) );
#elif defined SGDNN_BACKEND_2260
  sg_api_reciprocal_t api;
  api.dim = input.dim;
  ...
  api.input_global_addr = input.addr;
  api.output_global_addr = output.addr;
  SAFE_CALL ( sgdnnTPUKernelLaunch ( handle, "tpu_kernel_api_reciprocal_multi_core", &api, sizeof( api ) ) );
#else
  SGDNN_CHECK( false );
#endif
...
bm_status_t sgdnnTrunc ( bm_handle_t handle,
  ...
  api.input_global_addr = input.addr;
  api.output_global_addr = output.addr;
  SAFE_CALL ( sgdnnTPUKernelLaunch ( handle, "tpu_kernel_api_trunc", &api, sizeof( api ) ) );
#elif defined SGDNN_BACKEND_2260
  sg_api_trunc_t api;
  api.dim = input.dim;
  for(int i = 0; i < api.dim; ++i) {
  ...
  api.input_global_addr = input.addr;
  api.output_global_addr = output.addr;
  SAFE_CALL ( sgdnnTPUKernelLaunch ( handle, "tpu_kernel_api_trunc_multi_core", &api, sizeof( api ) ) );
#else
  SGDNN_CHECK( false );
#endif
  return BM_SUCCESS;

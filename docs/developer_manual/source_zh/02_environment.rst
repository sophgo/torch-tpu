...
注意：
1. TORCH-TPU工程在docker中的路径应该是/workspace/tpu-train
2. 如在CMODEL模式下进行开发，无需映射`/opt`目录
...
TORCH-TPU的代码依赖于PyTorch，因此需要提前安装PyTorch。在docker容器中已经安装好了pytorch==2.8.0，
...
SE7交叉编译是指在x86主机上编译出SE7平台上可安装执行的wheel包。SE7平台是一款基于BM1684X芯片的AI加速边缘设备，其架构与arm不同，因此需要在x86上交叉编译。

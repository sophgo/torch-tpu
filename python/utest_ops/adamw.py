...
def test():
    ...
    def standard_adam_update(a, b, lr, beta1, beta2, weight_decay, eps, amsgrad=False, maximize=False):
        optimizer = torch.optim.AdamW(a, lr=lr, betas=(beta1, beta2), weight_decay=weight_decay, eps=eps, amsgrad=amsgrad)
        optimizer.zero_grad()
    ...
    def tpu_adam(a, b, lr, beta1, beta2, weight_decay, eps, amsgrad=False, maximize=False):
        optimizer = torch.optim.AdamW(a, lr=lr, betas=(beta1, beta2), weight_decay=weight_decay, eps=eps, amsgrad=amsgrad, fused=True)
        optimizer.zero_grad()
        for i in range(len(a)):
            a[i].grad = b[i]
        optimizer.step()
    ...
    if not status:
        print("Optimizer AdamW is wrong!")
        sys.exit(255)
    else:

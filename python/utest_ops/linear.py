...
#                 self.f = nn.Linear(hidden_size, 3 * hidden_size)
#             def forward(self, a1):
#                 return self.f(a1) 
...
#         grad_o = torch.rand(batch, sequence, hidden_size*3)
#         grad_o_tpu =  move_to(grad_o, device, dtype)  #grad_o.to(device)

#         output_cpu.backward(grad_o)
#         output_tpu.backward(grad_o_tpu)
#         return input_sample_cpu[0].grad,input_sample_tpu[0].grad #Notice [0] because input_data has [],[]

...
#  if (len(output_cpu)==1):
 ...
 for i in range(num_output_tpu):
   ...
   temp_result = { "metric_value":dict_each_output_value, "metric_flag":dict_each_output_CMP_flag}
   dict+=[temp_result]
 return dict


# test a single input
def Torch_Test_Forward_Function_Per(case_name, module_native, input_sample,  metric_table = ['max_diff','MAE']
               , epsilon_dict = {'f32':1e-6,'f16':1e-2,'i32':1e-6}, seed=1000):\
    ...
    for naive_dtype in naive_dtype_list:
       ...
       if (temp_result):
         count = count +1
         print(case_name, "{} is success".format(naive_dtype), final_result[naive_dtype][0]['metric_value'])
         is_correct_dtype[naive_dtype] = 1
       else:
         print(case_name, "{} is failed: ".format(naive_dtype), final_result[naive_dtype][0]['metric_value'])
...
def Torch_Test_Forward_Function(case_name, module_native, input_sample_collection,  metric_table = ['max_diff','MAE']
    ...
    # print(("*****************Test {} Start*************************").format(case_name))
    ...
    for idx, input_sample in enumerate(input_sample_processed):
        ...
        temp_flag,is_correct_dtype = Torch_Test_Forward_Function_Per(case_name, module_native, input_sample,  metric_table
               , epsilon_dict, seed)

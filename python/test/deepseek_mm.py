...
class DeepseekMm(nn.Module):
    def __init__(self, w0, s0, blocksize):
        super().__init__()
        self.w0 = w0
        self.s0 = s0
...
class DeepseekMmFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, w0, s0, blocksize):
        output = torch.empty((x.shape[0], w0.shape[0]), dtype = x.dtype, device = x.device)

        torch.ops.my_ops.mm_w8a16_dq_forward(x,
                                    w0,
                                    s0,
                                    output,
                                    blocksize)
...
class DeepseekMmBlock(nn.Module):
    def __init__(self, w0, s0, blocksize):
        super().__init__()
        self.w0 = w0
        self.s0 = s0
    ...
    def forward(self, x):
        return DeepseekMmFunc.apply(x, self.w0, self.s0, self.blocksize)
...
def check_mlp():
    ...
    net_cpu = DeepseekMm(w0, s0, blocksize)
    ...
    # import pdb
    # pdb.set_trace()
    net_tpu = DeepseekMmBlock(w0_tpu, s0_tpu, blocksize)

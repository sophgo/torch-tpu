...
class GPT2Block(nn.Module):
    def __init__(self, config):
        ...
        self.ln_fc = lnMatmulBlock(self.embed_dim, 3 * self.embed_dim)
        self.add_ln_fc = AddlnMatmulBlock(config.hidden_size, inner_dim)
    ...
    def forward(
        ...
        _, _, x = self.ln_fc(hidden_states)
        # hidden_states = self.ln_1(hidden_states)
        # x_c = self.c_attn(hidden_states)

...
void bm_device::cmodel_setup(void)
{
  ...
  if (cmodel_deinit_ == NULL) {
      const char* cmodel_path = getenv("BMLIB_CMODEL_PATH");
      if (!cmodel_path) cmodel_path = "libcmodel.so";
      printf("loading cmodel: %s\n", cmodel_path);
      void *cmodel_so_handle_ = dlopen(cmodel_path, RTLD_LAZY);
      if(!cmodel_so_handle_) {
        printf("fail to load cmodel: %s\n", cmodel_path);
        return;
      }

set(sophon_DEPENDENCY_LIBS "")
set(sophon_cuda_DEPENDENCY_LIBS "")
set(sophon_hip_DEPENDENCY_LIBS "")
...
if(USE_REDIS)
  ...
  if(HIREDIS_FOUND)
    ...
    list(APPEND sophon_DEPENDENCY_LIBS ${HIREDIS_LIBRARIES})
  else()
...
if(USE_IBVERBS)
  ...
  if(IBVERBS_FOUND)
    ...
    list(APPEND sophon_DEPENDENCY_LIBS ${IBVERBS_LIBRARIES})
  else()
...
if(USE_LIBUV)
  ...
  if(TARGET uv_a)
    ...
    # Sophon build from your own project's build, and include libuv
...
if(USE_MPI)
  ...
  if(MPI_C_FOUND)
    ...
    list(APPEND sophon_DEPENDENCY_LIBS ${MPI_CXX_LIBRARIES})
    add_definitions(-DSOPHON_USE_MPI=1)
  else()
...
if(USE_CUDA AND USE_NCCL)
  ...
  if(NCCL_EXTERNAL)
    ...
    list(APPEND sophon_cuda_DEPENDENCY_LIBS ${NCCL_LIBRARIES} dl rt)
  else()
...
if(USE_ROCM)
  ...
  if(HAVE_HIP)
    ...
    foreach(sophon_rocm_arch ${SOPHON_ROCM_ARCH})
      list(APPEND HIP_CLANG_FLAGS --offload-arch=${sophon_rocm_arch})
    endforeach()

    set(SOPHON_HIP_INCLUDE ${hip_INCLUDE_DIRS} $<BUILD_INTERFACE:${HIPIFY_OUTPUT_ROOT_DIR}> $<INSTALL_INTERFACE:include> ${SOPHON_HIP_INCLUDE})

    # This is needed for library added by hip_add_library (same for hip_add_executable)
    hip_include_directories(${SOPHON_HIP_INCLUDE})

    set(sophon_hip_DEPENDENCY_LIBS ${SOPHON_HIP_HCC_LIBRARIES})
...
if(USE_ROCM AND USE_RCCL)
  ...
  if(RCCL_FOUND)
    ...
    list(APPEND sophon_hip_DEPENDENCY_LIBS ${RCCL_LIBRARIES} dl rt)
  else()

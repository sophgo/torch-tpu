...
ContextFactory::ContextFactory(std::shared_ptr<::sophon::Context> backingContext)
  ...
  for (auto i = 0; i < backingContext_->size; i++) {
    ...
    try {
      ...
      SOPHON_THROW("Backing context not fully connected");
    }
  }
...
std::shared_ptr<::sophon::Context> ContextFactory::makeContext(
    std::shared_ptr<transport::Device>& dev) {
  ...
  for (auto i = 0; i < context->size; i++) {
    ...
    SOPHON_ENFORCE_LE(addressSize, sendData_[i].size());
  ...
  return std::static_pointer_cast<::sophon::Context>(context);
}

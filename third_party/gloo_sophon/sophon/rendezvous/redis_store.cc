...
RedisStore::RedisStore(const std::string& host, int port) {
  ...
  SOPHON_ENFORCE(redis_ != nullptr);
  if (redis_->err != 0) {
    SOPHON_THROW_IO_EXCEPTION("Connecting to Redis: ", redis_->errstr);
  }
}
...
void RedisStore::set(const std::string& key, const std::vector<char>& data) {
  void* ptr = redisCommand(
      ...
      (size_t)data.size());
  if (ptr == nullptr) {
    SOPHON_THROW_IO_EXCEPTION(redis_->errstr);
  }
  redisReply* reply = static_cast<redisReply*>(ptr);
  if (reply->type == REDIS_REPLY_ERROR) {
    SOPHON_THROW_IO_EXCEPTION("Error: ", reply->str);
  }
  SOPHON_ENFORCE_EQ(reply->type, REDIS_REPLY_INTEGER);
  SOPHON_ENFORCE_EQ(reply->integer, 1, "Key '", key, "' already set");
...
std::vector<char> RedisStore::get(const std::string& key) {
  ...
  SOPHON_ENFORCE_EQ(reply->type, REDIS_REPLY_STRING);

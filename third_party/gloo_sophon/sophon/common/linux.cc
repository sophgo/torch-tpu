 ...
 * https://github.com/facebookincubator/sophon/pull/127
 */
...
const std::set<std::string>& kernelModules() {
  ...
  std::call_once(once, [](){
      ...
      while (std::getline(ifs, line)) {
        auto sep = line.find(' ');
        SOPHON_ENFORCE_NE(sep, std::string::npos);
...
static std::vector<std::string> listDir(const std::string& path) {
  ...
  SOPHON_ENFORCE(dirp != nullptr, strerror(errno));
  errno = 0;
  ...
  SOPHON_ENFORCE(errno == 0, strerror(errno));
  auto rv = closedir(dirp);
  SOPHON_ENFORCE(rv == 0, strerror(errno));
  return result;
}

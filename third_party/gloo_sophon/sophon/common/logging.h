...
#define SOPHON_ENFORCE(condition, ...)        \
  do {                                      \
    if (!(condition)) {                     \
      throw ::sophon::EnforceNotMet(          \
          __FILE__,                         \
          __LINE__,                         \
          #condition,                       \
          ::sophon::MakeString(__VA_ARGS__)); \
    }                                       \
  } while (false)
 ...
 *
 * SOPHON_ENFORCE_THAT can be used with one of the "checker functions" that
 * capture input argument values and add it to the exception message. E.g.
 * `SOPHON_ENFORCE_THAT(Equals(foo(x), bar(y)), "Optional additional message")`
 ...
 * functions to sophon::enforce_detail namespace. For example:
 *
 *   namespace sophon { namespace enforce_detail {
 ...
 *   }}
 *
 * With further usages like `SOPHON_ENFORCE_THAT(IsVector(Input(0).dims()))`
 *
 * Convenient wrappers for binary operations like SOPHON_ENFORCE_EQ are provided
 ...
 public:
  ...
  std::string get_message_and_free(std::string&& extra) const {
    ...
    if (extra.empty()) {
      ...
      r = ::sophon::MakeString(std::move(*msg_), ". ", std::move(extra));
    }
    delete msg_;
    return r;
  }
...
#define SOPHON_ENFORCE_THAT_IMPL(condition, expr, ...)        \
  do {                                                      \
    using namespace ::sophon::enforce_detail;                 \
...
#define SOPHON_ENFORCE_THAT(condition, ...) \
  SOPHON_ENFORCE_THAT_IMPL((condition), #condition, __VA_ARGS__)

#define SOPHON_ENFORCE_EQ(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(Equals((x), (y)), #x " == " #y, __VA_ARGS__)
#define SOPHON_ENFORCE_NE(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(NotEquals((x), (y)), #x " != " #y, __VA_ARGS__)
#define SOPHON_ENFORCE_LE(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(LessEquals((x), (y)), #x " <= " #y, __VA_ARGS__)
#define SOPHON_ENFORCE_LT(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(Less((x), (y)), #x " < " #y, __VA_ARGS__)
#define SOPHON_ENFORCE_GE(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(GreaterEquals((x), (y)), #x " >= " #y, __VA_ARGS__)
#define SOPHON_ENFORCE_GT(x, y, ...) \
  SOPHON_ENFORCE_THAT_IMPL(Greater((x), (y)), #x " > " #y, __VA_ARGS__)

...
void reduce(ReduceOptions &opts) {
  ...
  const size_t numSegments =
      roundUp(std::max((totalBytes + (segmentBytes - 1)) / segmentBytes,
  ...
  for (auto i = 0; i < numSegments; i++) {
    if (i >= 2) {
      ...
      if (prev.recvLength > 0) {
        ...
        opts.reduce(static_cast<uint8_t *>(out->ptr) + prev.recvOffset,
  ...
  if (context->rank == opts.root) {
    ...
    for (size_t rank = 0; rank < context->size; rank++) {
      ...
      ssize_t recvLength = std::min((ssize_t)chunkBytes,
    ...
    ssize_t sendLength = std::min((ssize_t)chunkBytes,

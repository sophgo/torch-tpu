...
Device::Device(const struct attr& attr, ibv_context* context)
  ...
  SOPHON_ENFORCE_EQ(rv, 0, "ibv_query_device: ", strerror(errno));

  // Query and store port attributes
  rv = ibv_query_port(context_, attr_.port, &portAttr_);
  SOPHON_ENFORCE_EQ(rv, 0, "ibv_query_port: ", strerror(errno));
...
void Device::loop() {
  ...
  while (!done_) {
    ...
    SOPHON_ENFORCE_EQ(rv, 0, "ibv_get_cq_event");

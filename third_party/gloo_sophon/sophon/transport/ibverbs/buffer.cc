...
Buffer::Buffer(Pair* pair, int slot, void* ptr, size_t size)
    : ::sophon::transport::Buffer(slot, ptr, size),
      pair_(pair),
      recvCompletions_(0),
      sendCompletions_(0),
      sendPending_(0),
  ...
  SOPHON_ENFORCE(mr_ != nullptr, "ibv_reg_mr: ", strerror(errno));
}

Buffer::~Buffer() {
  SOPHON_ENFORCE_EQ(sendPending_, 0, "Destructing buffer expecting completions");
  ibv_dereg_mr(mr_);
}
    ...
    while (recvCompletions_ == 0) {
      ...
      if (timeout != kNoTimeout &&
        ...
        pair_->signalIoFailure(
          SOPHON_ERROR_MSG("Read timeout ", pair_->peer().str()));
...
void Buffer::waitSend() {
  ...
  if (pair_->sync_) {
    ...
    if (sendCompletions_ == 0) {
      SOPHON_ENFORCE_GT(sendPending_, 0, "No send to wait for");
      auto start = std::chrono::steady_clock::now();
      // We can assume a single pair is never used by more than one
      // thread, so there is no need to acquire the mutex here.
      while (sendCompletions_ == 0) {
        ...
        if (timeout != kNoTimeout &&
            (std::chrono::steady_clock::now() - start) >= timeout) {
          pair_->signalIoFailure(
            SOPHON_ERROR_MSG("Send timeout ", pair_->peer().str()));
          SOPHON_ENFORCE(false, "Unexpected code path");
        }
      }
    }
...
void Buffer::send(size_t offset, size_t length, size_t roffset) {
  ...
  // to support this.
  SOPHON_ENFORCE_LE(offset + length, size_);
...
void Buffer::handleCompletion(struct ibv_wc* wc) {
  if (wc->opcode & IBV_WC_RECV) {
    ...
    SOPHON_ENFORCE(false, "Unexpected completion (opcode: ", wc->opcode, ")");
  }
}

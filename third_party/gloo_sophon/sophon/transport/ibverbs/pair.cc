...
#include "sophon/transport/ibverbs/buffer.h"
...
void Pair::sendMemoryRegion(struct ibv_mr* src, int slot) {
  ...
  if (rv != 0) {
    signalIoFailure(SOPHON_ERROR_MSG("ibv_post_send: ", rv));
  }
  ...
  SOPHON_ENFORCE_EQ(mappedSendRegions_.count(slot), 0);
...
const struct ibv_mr* Pair::getMemoryRegion(int slot) {
  ...
  if (sync_) {
      ...
      if (timeout_ != kNoTimeout &&
        ...
        signalIoFailure(
            SOPHON_ERROR_MSG(
                ...
                peer_.str()));
        SOPHON_ENFORCE(false, "Unexpected code path");
      }
    ...
    auto it = peerMemoryRegions_.find(slot);
    SOPHON_ENFORCE(it != peerMemoryRegions_.end());
    return &it->second;
  }
}
...
void Pair::postReceive() {
  ...
  if (rv != 0) {
    signalIoFailure(SOPHON_ERROR_MSG("ibv_post_recv: ", rv));
  }
}

std::unique_ptr<::sophon::transport::Buffer>
Pair::createSendBuffer(int slot, void* ptr, size_t size) {
  std::unique_lock<std::mutex> lock(m_);
  SOPHON_ENFORCE_EQ(sendCompletionHandlers_.count(slot), 0);
...
Pair::createRecvBuffer(int slot, void* ptr, size_t size) {
  std::unique_lock<std::mutex> lock(m_);
  SOPHON_ENFORCE_EQ(recvCompletionHandlers_.count(slot), 0);
  auto buffer = new Buffer(this, slot, ptr, size);
...
void Pair::handleCompletion(struct ibv_wc* wc) {
  if (wc->opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
    ...
    SOPHON_ENFORCE(recvCompletionHandlers_[slot] != nullptr);
    ...
    SOPHON_ENFORCE(sendCompletionHandlers_[slot] != nullptr);
    ...
    SOPHON_ENFORCE_GT(mappedSendRegions_.size(), 0);
    SOPHON_ENFORCE_EQ(mappedSendRegions_.count(slot), 1);
    mappedSendRegions_.erase(slot);
  } else {
    SOPHON_ENFORCE(false, "Unexpected completion with opcode: ", wc->opcode);
  }
}
...
void Pair::send(Buffer* buffer, size_t offset, size_t length, size_t roffset) {
  ...
  SOPHON_ENFORCE_NE(peer, (const struct ibv_mr*)nullptr);
...
void Pair::signalIoFailure(const std::string& msg) {
  ...
  auto ex = ::sophon::IoException(msg);
  if (ex_ == nullptr) {
    ...
    for (auto& it : recvCompletionHandlers_) {
      SOPHON_ENFORCE(it.second != nullptr);

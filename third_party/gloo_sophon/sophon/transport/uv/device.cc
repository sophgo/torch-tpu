...
static void lookupAddrForHostname(struct attr& attr) {
  ...
  SOPHON_ENFORCE(rp != nullptr, "Unable to find address for: ", attr.hostname);
  freeaddrinfo(result);
  return;
}
...
std::shared_ptr<transport::Device> CreateDevice(struct attr attr) {
  if (!attr.iface.empty()) {
    ...
    if (!lookupAddrForIface(&attr)) {
      SOPHON_ENFORCE(false, "Unable to find address for: ", attr.iface);
    }
  } else {
...
Device::Device(const struct attr& attr) : attr_(attr) {
  ...
  listener_->on<libuv::ErrorEvent>(
      [this](const libuv::ErrorEvent& event, const libuv::TCP&) {
        ...
        SOPHON_ENFORCE(!event, "Error on listener socket: ", event.what());
      });
...
void Device::connect(
  ...
  SOPHON_ENFORCE_EQ(ss1.ss_family, ss2.ss_family);

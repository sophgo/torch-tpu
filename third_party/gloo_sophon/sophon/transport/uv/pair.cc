...
#include <sophon/transport/uv/pair.h>
...
#include <sophon/transport/uv/common.h>
#include <sophon/transport/uv/context.h>
#include <sophon/transport/uv/libuv.h>
#include <sophon/transport/uv/unbound_buffer.h>
...
void Pair::connect(const std::vector<char>& bytes) {
  ...
  std::unique_lock<std::mutex> lock(mutex_);
  SOPHON_ENFORCE_EQ(state_, INITIALIZED);
  state_ = CONNECTING;
  ...
  if (errno_) {
    throw ::sophon::IoException(SOPHON_ERROR_MSG(
        "Error connecting to ",
        peer.str(),
        ": ",
...
void Pair::onRead(const libuv::ReadEvent& event, const libuv::TCP&) {
  ...
  if (op.nread == 0) {
    SOPHON_ENFORCE(event.length == sizeof(op.preamble));
  }
  ...
  if (opcode == Op::SEND_UNBOUND_BUFFER) {
    ...
    if (!op.buf) {
      ...
      SOPHON_ENFORCE(op.buf, "Cannot lock pointer to unbound buffer");
    ...
    SOPHON_ENFORCE(op.nread == op.preamble.nbytes);
...
void Pair::onNotifyRecvReady(const Op& op) {
  ...
  if (it != localPendingSend_.end()) {
    ...
    SOPHON_ENFORCE(buf, "Cannot lock pointer to unbound buffer");

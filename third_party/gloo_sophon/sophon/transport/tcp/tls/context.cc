...
#include "sophon/transport/tcp/tls/openssl.h"
...
std::string getSSLErrorMessage() {
  std::stringstream ss;
  _sophontls::ERR_print_errors_cb(
...
Context::Context(std::shared_ptr<Device> device, int rank, int size)
    : ::sophon::transport::tcp::Context(
          std::dynamic_pointer_cast<::sophon::transport::tcp::Device>(device),
          rank, size),
      ssl_ctx_(create_ssl_ctx(c_str_or_null(device->getPKeyFile()),
               ...
               [](::SSL_CTX *x) { ::_sophontls::SSL_CTX_free(x); }) {}

SSL_CTX *Context::create_ssl_ctx(const char *pkey, const char *cert,
                                 const char *ca_file, const char *ca_path) {
  SOPHON_ENFORCE(pkey != nullptr && cert != nullptr,
               "Private key and certificate location must be specified");
  SOPHON_ENFORCE(ca_file != nullptr || ca_path != nullptr,
               "CAfile or CApath must be specified");
  static std::once_flag ssl_ctx_init_;
  std::call_once(ssl_ctx_init_, [] {
    //    SSL_load_error_strings();
    //    SSL_library_init();
    _sophontls::OPENSSL_init_ssl(
        OPENSSL_INIT_LOAD_SSL_STRINGS | OPENSSL_INIT_LOAD_CRYPTO_STRINGS, NULL);
    _sophontls::OPENSSL_init_ssl(0, NULL);
  });
  SSL_CTX *ssl_ctx = _sophontls::SSL_CTX_new(_sophontls::TLS_method());
  SOPHON_ENFORCE(ssl_ctx != nullptr, getSSLErrorMessage());
  SOPHON_ENFORCE(
      _sophontls::SSL_CTX_set_min_proto_version(ssl_ctx, TLS_MAX_VERSION) == 1,
      getSSLErrorMessage());

  // As we don't need to handle legacy clients,
  // let's remove support for legacy renegotiation:
  _sophontls::SSL_CTX_clear_options(ssl_ctx, SSL_OP_LEGACY_SERVER_CONNECT);

  _sophontls::SSL_CTX_set_verify_depth(ssl_ctx, 1);
  ...
  _sophontls::SSL_CTX_set_security_level(ssl_ctx, 2);

  SOPHON_ENFORCE(
      _sophontls::SSL_CTX_load_verify_locations(ssl_ctx, ca_file, ca_path) == 1,
      getSSLErrorMessage());
  SOPHON_ENFORCE(_sophontls::SSL_CTX_use_certificate_chain_file(ssl_ctx, cert) == 1,
               getSSLErrorMessage());
  SOPHON_ENFORCE(_sophontls::SSL_CTX_use_PrivateKey_file(ssl_ctx, pkey,
                                                     SSL_FILETYPE_PEM) == 1,
               getSSLErrorMessage());
  SOPHON_ENFORCE(_sophontls::SSL_CTX_check_private_key(ssl_ctx) == 1,
               getSSLErrorMessage());
  // SSL_VERIFY_PEER
  //
  ...
  _sophontls::SSL_CTX_set_verify(ssl_ctx,
                               SSL_VERIFY_PEER |

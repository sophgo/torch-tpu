...
#include "sophon/transport/tcp/helpers.h"
...
static void lookupAddrForIface(struct attr& attr) {
  ...
  SOPHON_ENFORCE_NE(rv, -1, strerror(errno));
  struct ifaddrs *ifa;
  for (ifa = ifap; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    switch (attr.ai_family) {
      ...
      // ifa->ifa_addr->sa_family: 地址类型，例如IPv4(AF_INET) IPv6(AF_INET6) 本地进程间通信(AF_UNIX) 链路层地址(AF_LINK)
        ...
        break;
      default:
        SOPHON_ENFORCE(false, "Unknown ai_family: ", attr.ai_family);
        break;
    }
...
bool Device::isInitiator(
  ...
  if (family == AF_INET) {
    ...
    SOPHON_ENFORCE(false, "Unknown address family: ", family);
  }
  ...
  SOPHON_ENFORCE_NE(rv, 0, "Cannot connect to self");
  return rv > 0;
}

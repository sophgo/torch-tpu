...
#include "sophon/common/memory.h"
...
class Pair : public ::sophon::transport::Pair, public Handler {
 protected:
  enum state {
    INITIALIZING = 1,
    CONNECTING = 3,
 ...
 public:
  ...
  virtual std::unique_ptr<::sophon::transport::Buffer> createSendBuffer(
      int slot,
      void* ptr,
      size_t size) override;

  virtual std::unique_ptr<::sophon::transport::Buffer> createRecvBuffer(
      int slot,
      void* ptr,
      size_t size) override;
 ...
 protected:
  ...
  void waitUntil(pred_t pred, std::unique_lock<std::mutex>& lock,
    ...
    if (useTimeout && timeoutSet) {
      ...
      if (!done) {
        signalAndThrowException(SOPHON_ERROR_MSG("Connect timeout ", peer_.str()));
      }
    } else {
      cv_.wait(lock, pred);
    }

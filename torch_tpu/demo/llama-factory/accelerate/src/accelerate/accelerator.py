...
class Accelerator:
    ...
    **Available attributes:**
    ...
    def __init__(
        ...
        # self.cp_dynamo_plugin = dynamo_plugin
        ...
        if (
            ...
            if self.device.type not in ("cuda", "mps", "npu","tpu"):
    ...
    def prepare(self, *args, device_placement=None):
        ...
        if not hasattr(self.state, "distributed_type") : self.state.distributed_type = DistributedType.NO
        if self.distributed_type == DistributedType.FSDP:
            model_count = 0
        ...
        # if hasattr(self.state, "mixed_precision"): self.state.mixed_precision == "no"
    ...
    def prepare_model(self, model: torch.nn.Module, device_placement: bool = None, evaluation_mode: bool = False):
        ...
        if (getattr(model, "is_loaded_in_8bit", False) or getattr(model, "is_loaded_in_4bit", False)) and getattr(
            ...
            if int(os.environ.get("MODEL_IN_DEVICE", 0)) == 1:
                pass
            else:
    ...
    def clip_grad_norm_(self, parameters, max_norm, norm_type=2):
        ...
        self.unscale_gradients()
        if int(os.environ.get('USE_SOPHGO', 0)) == 1 and int(os.environ.get('SOPHGO_SPLIT', 0)) > 1:
            # for para in parameters:
            #     torch.nn.utils.clip_grad_norm_(para, max_norm, norm_type=norm_type)
            # return 
            ...
            if parameters:
                ...
                for device in devices:
                    # 获取在当前设备上的参数
                    params_on_device = [p for p in parameters if p.device == device]
                    # 将参数转换为一个向量
                    # flattened_grads = torch.nn.utils.parameters_to_vector(params_on_device)
                    # 对向量进行梯度裁剪
                    torch.nn.utils.clip_grad_norm_(params_on_device, max_norm, norm_type=norm_type)
                    # 将裁剪后的梯度重新分配回各个参数
                    # torch.nn.utils.vector_to_parameters(flattened_grads, params_on_device)
            return 

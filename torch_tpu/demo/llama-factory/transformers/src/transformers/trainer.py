...
def check_sophgo_tpu():
    import os
    return int(os.environ.get('USE_SOPHGO', 0)) == 1
...
class Trainer:
    ...
    def _move_model_to_device(self, model, device):
        ...
        def model_to_device2(model,devices):
            submodel = model.model.model
            submodel.embed_tokens.to(devices[0])
            submodel.layers[:16].to(devices[0])
            submodel.layers[16:].to(devices[-1])
            submodel.norm.to(devices[-1])
            model.model.lm_head.to(devices[-1])
            return model

        def model_to_device_llama_13b(model, devices):
            assert(len(devices) == 4)
            submodel = model.model.model
            submodel.embed_tokens.to(devices[0])
            submodel.layers[:10].to(devices[0])
            submodel.layers[10:20].to(devices[1])
            submodel.layers[20:30].to(devices[2])
            submodel.layers[30:].to(devices[3])
            submodel.norm.to(devices[3])
            model.model.lm_head.to(devices[3])
            return model

        def model_to_device_llama_7b_allfinetune(model, devices):
            submodel = model.model
            submodel.embed_tokens.to(devices[0])
            for i in range(0,32,3):
                submodel.layers[i:i+3].to(devices[i//3])
            submodel.norm.to(devices[10])
            model.lm_head.to(devices[10])
            return model

        def model_to_device_full_train_llama_13b(model, devices):
            assert(len(devices)==21)
            submodel = model.model
            submodel = model.model
            submodel.embed_tokens.to(devices[0])
            submodel.layers[0].to(devices[0])
            step = 1
            for i in range(1,39,2):
                submodel.layers[i].to(devices[step])
                submodel.layers[i+1].to(devices[step])
                step += 1
            submodel.layers[39].to(devices[step])
            submodel.norm.to(devices[step])
            model.lm_head.to(devices[step])
            return model

        def move_model_to_device(model, device):
            use_sophgo = check_sophgo_tpu()
            sophgo_split = int(os.environ.get('SOPHGO_SPLIT', 0))
            sophgo_test = int(os.environ.get('SOPHGO_TEST', 0))
            if use_sophgo and not sophgo_test:
                if sophgo_split == 2:
                    device2 = "tpu:" + str(int(str(device).split(":")[1])+1)
                    model = model_to_device2(model, [device, device2])
                    os.environ['MODEL_IN_DEVICE']='1'
                elif sophgo_split == 4:
                    devices = ["tpu:" + str(int(str(device).split(":")[1])+i) for i in range(4)]
                    model = model_to_device_llama_13b(model, devices)
                    os.environ['MODEL_IN_DEVICE']='1'
                elif sophgo_split == 12:
                    devices = ["tpu:" + str(int(str(device).split(":")[1])+i) for i in range(12)]
                    model = model_to_device_llama_7b_allfinetune(model, devices)
                    os.environ['MODEL_IN_DEVICE']='1'
                elif sophgo_split == 21:
                    devices = ["tpu:" + str(int(str(device).split(":")[1])+i) for i in range(21)]
                    model = model_to_device_full_train_llama_13b(model, devices)
                    os.environ['MODEL_IN_DEVICE']='1'
                else:
                    print("default model to device all move into one device, please pay attention!")
    ...
    def get_optimizer_cls_and_kwargs(args: TrainingArguments) -> Tuple[Any, Any]:
        ...
        if int(os.environ.get('USE_SOPHGO', 0)) == 1:
    ...
    def _inner_training_loop(
        ...
        tpu_split = int(os.environ.get('SOPHGO_SPLIT', 0))
        if int(os.environ.get('USE_SOPHGO', 0)) == 1 and tpu_split >= 2:
            device2 = "tpu:" + str(int(str(args.device).split(":")[1])+tpu_split-1)
            tr_loss = torch.tensor(0.0).to(device2)
        else:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    and (torch.isnan(tr_loss_step.cpu()) or torch.isinf(tr_loss_step.cpu()))
                ):

...
class AttentionMaskConverter:
    ...
    def _make_causal_mask(
        ...
        mask = torch.full((tgt_len, tgt_len), torch.iinfo(torch.int32).min, device=device, dtype=torch.int32)
        mask_cond = torch.arange(mask.size(-1), device=device, dtype=torch.int32)
        t = mask_cond < (mask_cond + 1).view(mask.size(-1), 1)
        
        mask.masked_fill_(t, 0)

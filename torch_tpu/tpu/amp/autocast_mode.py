...
class autocast(torch.amp.autocast_mode.autocast):
    ...
    ``torch.tpu.amp.autocast(args...)`` is equivalent to ``torch.autocast("tpu", args...)``
    """
    ...
    def __init__(self, enabled: bool = True, dtype: torch.dtype = torch.float16, cache_enabled: bool = True):
        if torch._jit_internal.is_scripting():
            ...
            return
        super().__init__("tpu", enabled=enabled, dtype=dtype, cache_enabled=cache_enabled)
...
def _cast(value, dtype):
    if isinstance(value, torch.Tensor):
        is_eligible = (value.is_floating_point() and value.device.type == 'tpu' and (value.dtype is not torch.float64))
...
def custom_fwd(fwd=None, **kwargs):
    ...
    def decorate_fwd(*args, **kwargs):
        if cast_inputs is None:
            args[0]._fwd_used_autocast = torch_tpu._C.is_autocast_enabled()
            return fwd(*args, **kwargs)
        else:
            autocast_context = torch_tpu._C.is_autocast_enabled()

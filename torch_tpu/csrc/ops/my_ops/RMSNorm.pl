...
template <typename T>
void rmsnorm_kernel(T *ptr_output, T *ptr_input, T *ptr_weight, float eps,
                    bool affine, int outer_size, int inner_size, const int block_w) {
    int N = 1;
    int C = outer_size;
    int H = 1;
    int W = inner_size;
...
__KERNEL__ void rmsnorm_fp32(fp32 *ptr_output, fp32 *ptr_input,
                             fp32 *ptr_weight, float eps, bool affine,
                             int outer_size, int inner_size, const int block_w) {
    rmsnorm_kernel<fp32>(ptr_output, ptr_input, ptr_weight, eps, affine, outer_size,
                         inner_size, block_w);
}

__KERNEL__ void rmsnorm_fp16(fp16 *ptr_output, fp16 *ptr_input,
                             fp16 *ptr_weight, float eps, bool affine,
                             int outer_size, int inner_size, const int block_w) {
    rmsnorm_kernel<fp16>(ptr_output, ptr_input, ptr_weight, eps, affine, outer_size,
                         inner_size, block_w);
}

__KERNEL__ void rmsnorm_bf16(bf16 *ptr_output, bf16 *ptr_input,
                             bf16 *ptr_weight, float eps, bool affine,
                             int outer_size, int inner_size, const int block_w) {
    rmsnorm_kernel<bf16>(ptr_output, ptr_input, ptr_weight, eps, affine, outer_size,
                         inner_size, block_w);
}

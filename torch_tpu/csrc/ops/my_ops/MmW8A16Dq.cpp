...
bool findValid_mn(uint32_t m_slice, uint32_t n_slice, std::function<int(tpuStream_t, tpuKernelModule_t, uint32_t, uint32_t)> kernel_func) {
...
bool findValid_k(uint32_t k_slice, std::function<int(tpuStream_t, tpuKernelModule_t, uint32_t)> kernel_func) {
  tpuStream_t stream = c10_tpu::getCurrentTPUStream().stream();
  tpuKernelModule_t ppl_module = getPplModule();
...

{
	...
	if (slice_m_n < slice_k) {
		...
		auto kernel_mn_mn = [&](tpuStream_t stream, tpuKernelModule_t ppl_module, uint32_t m_slice, uint32_t n_slice) -> int {
				return  matmul_w8a16_mn_mn_bf16(stream, ppl_module, output_addr,left_addr, right_addr, scale_addr,
		...
		auto kernel_mn_k = [&](tpuStream_t stream, tpuKernelModule_t ppl_module, uint32_t k_slice) -> int {
				return  matmul_w8a16_mn_k_bf16(stream, ppl_module, output_addr,left_addr, right_addr, scale_addr,
				M, K, N, group_size, m_slice, n_slice, k_slice, group_core);
		};

		if(K < percore_n){
			bool ret_k = findValid_k(k_slice, kernel_mn_k);
			if(ret_k) return;
			else{
				bool ret_mn = findValid_mn(m_slice, n_slice, kernel_mn_mn);
				if(ret_mn) return;
		...
		uint32_t  percore_k = ((K + CORE_NUM * group_size - 1) / (CORE_NUM * group_size)) * group_size;
		uint32_t m_slice = M;
		uint32_t n_slice = N;
		...
		auto kernel_k_mn = [&](tpuStream_t stream, tpuKernelModule_t ppl_module, uint32_t m_slice, uint32_t n_slice) -> int {
				return  matmul_w8a16_k_mn_bf16(stream, ppl_module, output_addr,left_addr, right_addr, scale_addr,
		...
		auto kernel_k_k = [&](tpuStream_t stream, tpuKernelModule_t ppl_module, uint32_t k_slice) -> int {
				return  matmul_w8a16_k_k_bf16(stream, ppl_module, output_addr,left_addr, right_addr, scale_addr,
							M, K, N, group_size, m_slice, n_slice, k_slice);
		};

		if(percore_k < N){
			bool ret_k = findValid_k(k_slice, kernel_k_k);
			if(ret_k) return;
			else{
				bool ret_mn = findValid_mn(m_slice, n_slice, kernel_k_mn);
				if(ret_mn) return;
				else TORCH_CHECK(false, "Tile size reduction failed after attempts");
			}
		}

...
// 2. topk(1, C, 1, k*iter_w)=(1, C, 1, k)
    ...
    dim4 block_x_trans_shape = {1, block_c, block_size, 1};
    dim4 block_x_trans_stride = {block_c * block_size, block_size, 1, 0};

    for (auto c_idx = 0; c_idx < slice_size_for_core; c_idx += block_c) {
        int c = block_c;
        auto ones_block = make_tensor<fp32>(block_shape, block_shape);
        auto zeros_block = make_tensor<fp32>(block_shape, block_shape);
        tiu::fill(ones_block, 1.0f);
        tiu::fill(zeros_block, 0.0f);

        dim4 pool_buffer_shape = {1, c, block_size, 1};
        padding_t pad = {0, 0, 0, 0};
        dim2 dil = {1,1}, ins = {0,0};
        dim2 kernel = {1, block_size}, stride = {1, 1};
        dim4 scatter_index_shape = {1, c, 1, block_size};
        dim4 scatter_out_shape = {1, c, 1, block_size};
        ...
        dim4 in_lshape = {1, c, 1, block_size};
        ...
        for (auto w_idx = 0; w_idx < W; w_idx += block_size) {
            ...
            auto x_fp32 = make_tensor<T>(in_lshape, in_lshape);
            dma::load(x_fp32, in_g.sub_view(in_lshape, in_offset));

    // 1.1) input + tiny_bias*i to avoid the same input value
            auto idx_u32 = make_tensor<uint32_t>(in_lshape, in_lshape);
            arange_broadcast(idx_u32, c, w_idx, 1, block_size);

            auto idx_fp32 = make_tensor<fp32>(in_lshape, in_lshape);
            tiu::cast(idx_fp32, idx_u32);

            // bias = idx * tiny_bias
            auto bias = make_tensor<fp32>(in_lshape, in_lshape);
            const float tiny_bias = 1e-6f;
            tiu::fmul(bias, idx_fp32, tiny_bias);

            // x_fp32 = x_fp32 + bias
            auto x = make_tensor<fp32>(in_lshape, in_lshape);
            tiu::fadd(x, x_fp32, bias);

    // 1.2) mask via gt_select
            dim4 block_offset = {0, 0, 0, w_idx};
            // auto x_block = x.sub_view(block_x_shape, block_offset);
            // auto idx_u32_block = idx_u32.sub_view(in_lshape, block_offset);
            auto Xcol = make_tensor<fp32>(block_shape, block_shape);
            auto Xrow = make_tensor<fp32>(block_shape, block_shape);
            tiu::fmul(Xcol, ones_block, x); // x: [1,1,1,block_size]

            auto x_row_view = x.view(block_x_trans_shape, block_x_trans_stride);
            tiu::fmul(Xrow, ones_block, x_row_view);

            auto mask = make_tensor<fp32>(block_shape, block_shape);
            if (largest) {
    ...
    // 1.3) avgpool
            auto pool_buffer = make_tensor<fp32>(pool_buffer_shape, pool_buffer_shape);
            tiu::fpool_avg(pool_buffer, mask, &kernel, &pad, &stride, &dil, &ins, 1.0f);

            auto pool_int =  make_tensor<uint16_t>(pool_buffer_shape, pool_buffer_shape);
            tiu::cast(pool_int, pool_buffer);

    // 1.4) scatter

            auto scatter_out = make_tensor<fp32>(scatter_out_shape, scatter_out_shape); // {1,C,1,W}

            tiu::scatter_w(scatter_out, x, pool_int.view(scatter_index_shape),0);// x -> {1,C,1,W} ; scatter_index_w -> {1,W,1,1}

            auto scatter_index_index_out = make_tensor<uint32_t>(scatter_out_shape, scatter_out_shape);
    ...
    if (iter_w == 1){  // one stage
        dma::store(outv_g.sub_view(out_lshape, in_offset_), outv_block.view(out_lshape));
    ...
    // 2.1) mask via gt_select
        ...
        auto out_ones_block = make_tensor<fp32>(mask_shape, mask_shape);
        auto out_zeros_block = make_tensor<fp32>(mask_shape, mask_shape);
        tiu::fill(out_ones_block, 1.0f);
        tiu::fill(out_zeros_block, 0.0f);

        auto out_Xcol = make_tensor<fp32>(mask_shape, mask_shape);
        auto out_Xrow = make_tensor<fp32>(mask_shape, mask_shape);
        auto Xrow = make_tensor<fp32>(mask_shape, mask_shape);
        tiu::fmul(out_Xcol, out_ones_block, outv_block); // x: [1,1,1,k*iter_w]

        dim4 block_topk_trans_stride = {c*k*iter_w, k*iter_w, 1, 0};
       ...
       auto out_mask = make_tensor<fp32>(mask_shape, mask_shape);
       if (largest) {
    ...
    //  2.2) avgpool
       ...
       auto out_pool_buffer = make_tensor<fp32>(out_pool_buffer_shape, out_pool_buffer_shape);
       tiu::fpool_avg(out_pool_buffer, out_mask, &out_kernel, &pad, &stride, &dil, &ins, 1.0f);

       auto out_pool_int =  make_tensor<uint16_t>(out_pool_buffer_shape, out_pool_buffer_shape);
       tiu::cast(out_pool_int, out_pool_buffer);

       auto out_scatter_index = make_tensor<uint16_t>(out_scatter_index_shape, out_scatter_index_shape);

    // 2.3) scatter
       auto out_scatter_out = make_tensor<fp32>(out_scatter_out_shape, out_scatter_out_shape); // {1,C,1,W}

       tiu::scatter_w(out_scatter_out, outv_block, out_pool_int.view(out_scatter_index_shape), 0);

       auto out_scatter_index_index_out = make_tensor<uint32_t>(out_scatter_out_shape, out_scatter_out_shape);
       tiu::scatter_w(out_scatter_index_index_out, outi_block, out_pool_int.view(out_scatter_index_shape), 0);

       dma::store(outv_g.sub_view(out_lshape, in_offset_), out_scatter_out.view(out_lshape));

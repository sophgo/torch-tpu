...
static bool findValidTiling_mn(uint32_t m_slice, uint32_t n_slice, std::function<int(TPUStream, tpuKernelModule_t, uint32_t, uint32_t)> kernel_func) {
...
static bool findValidTiling_k(uint32_t k_slice, std::function<int(TPUStream, tpuKernelModule_t, uint32_t)> kernel_func) {
  auto stream = c10_tpu::getCurrentTPUStream();
  tpuKernelModule_t ppl_module = getPplModule();
  ...
  bool has_bias)
{
  ...
  auto kernel_mn_mnk = [&](TPUStream stream, tpuKernelModule_t ppl_module, uint32_t m_slice, uint32_t n_slice, uint32_t k_slice) -> int {
      return  matmul_mn_mnk(stream,
        #ifndef BACKEND_SG2260
        ppl_module,
        #endif
        output_addr, left_addr, right_addr, bais_addr, M, K, N, m_slice, n_slice, k_slice, group_core, has_bias);
  };
  ...
  if(slice_m_n < slice_k) {
    ...
    kernel_k =  [&](TPUStream stream, tpuKernelModule_t ppl_module, uint32_t k_slice) -> int {
          return  matmul_mn_k(stream,
            #ifndef BACKEND_SG2260
            ppl_module,
            #endif
            output_addr, left_addr, right_addr, M, K, N, m_slice, n_slice, k_slice, group_core);
    };
  }
    ...
    kernel_mn = [&](TPUStream stream, tpuKernelModule_t ppl_module, uint32_t m_slice, uint32_t n_slice) -> int {
          return  matmul_k_mn(stream,
            #ifndef BACKEND_SG2260
            ppl_module,
            #endif
            output_addr, left_addr, right_addr, bais_addr, M, K, N, m_slice, n_slice, k_slice, has_bias);
    };

    kernel_k = [&](TPUStream stream, tpuKernelModule_t ppl_module, uint32_t k_slice) -> int {
          return  matmul_k_k(stream,
            #ifndef BACKEND_SG2260
            ppl_module,
            #endif
            output_addr, left_addr, right_addr, M, K, N, m_slice, n_slice, k_slice);
    };
  }
  ...
  bool ret_mn = findValidTiling_mn(m_slice, n_slice, kernel_mn);
  if(ret_mn) return;
  else{
    if (has_bias) kernel_mn_mnk(stream, ppl_module, 128, 128, 128);
    else {
      bool ret_k = findValidTiling_k(k_slice, kernel_k);
      if(ret_k) return;

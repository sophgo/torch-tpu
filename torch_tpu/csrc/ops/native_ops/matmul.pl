...
template <typename T>
void matmul_mn_mn_kernel(T* out_ptr, T* left_ptr, T* right_ptr, T* bias_ptr,
                         int M, int K, int N, const int m_slice, const int n_slice, const int k_slice,
                         const int group_core, const bool has_bias) {
...
void matmul_k_mn_kernel(
        ...
        const int m_slice, const int n_slice, const int k_slice, const bool has_bias) {
...
template <typename T>
void matmul_mn_mnk_kernel(T* out_ptr, T* left_ptr, T* right_ptr, T* bias_ptr, int M, int K, int N,
                         const int m_slice, const int n_slice, const int k_slice,
                         const int group_core, const bool has_bias) {
    ppl::set_core_num(CORE_NUM);
    int core_num = ppl::get_core_num();
...
__KERNEL__ void matmul_mn_mn( bf16* out_ptr, bf16* left_ptr, bf16* right_ptr, bf16* bias_ptr, int M, int K, int N, 
                            const int m_slice, const int n_slice, const int k_slice, const int group_core, const bool has_bias) {
    matmul_mn_mn_kernel(out_ptr, left_ptr, right_ptr, bias_ptr, M, K, N, m_slice, n_slice, k_slice, group_core, has_bias);
}
...
__KERNEL__ void matmul_k_mn( bf16* out_ptr, bf16* left_ptr, bf16* right_ptr, bf16* bias_ptr, int M, int K, int N,
                            const int m_slice, const int n_slice, const int k_slice, const bool has_bias) {
    matmul_k_mn_kernel(out_ptr, left_ptr, right_ptr, bias_ptr, M, K, N, m_slice, n_slice, k_slice, has_bias);
}

__KERNEL__ void matmul_k_k(bf16* out_ptr, bf16* left_ptr, bf16* right_ptr,int M, int K, int N, 
                            const int m_slice, const int n_slice, const int k_slice) {
    matmul_k_k_kernel(out_ptr, left_ptr, right_ptr,M, K, N, m_slice, n_slice, k_slice);
}

__KERNEL__ void matmul_mn_mnk(bf16* out_ptr, bf16* left_ptr, bf16* right_ptr, bf16* bias_ptr, int M, int K, int N,
                            const int m_slice, const int n_slice, const int k_slice, const int group_core, const bool has_bias) {
    matmul_mn_mnk_kernel(out_ptr, left_ptr, right_ptr, bias_ptr, M, K, N, m_slice, n_slice, k_slice, group_core, has_bias);
}

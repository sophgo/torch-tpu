                          ...
                          bool            track_running_status )
{
  ...
  auto input_tpu = input_cpu.to ( tpu::TPUGetCurrentDevice() ).to ( torch::kHalf );
  auto weight_tpu = weight_cpu.to ( tpu::TPUGetCurrentDevice() ).to ( torch::kHalf );
  auto bias_tpu = bias_cpu.to ( tpu::TPUGetCurrentDevice() ).to ( torch::kHalf );
  ...
  if ( track_running_status )
  {
    ...
    running_mean_tpu = running_mean_cpu.to ( tpu::TPUGetCurrentDevice() ).to ( torch::kHalf );
    running_var_tpu = running_var_cpu.to ( tpu::TPUGetCurrentDevice() ).to ( torch::kHalf );
  }
  ...
  auto outputs_tpu = torch::native_batch_norm (
                     ...
                     momentum,
                     eps );
  auto output_got = std::get<0> ( outputs_tpu ).to ( torch::kFloat ).to ( torch::Device ( "cpu" ) );
  auto save_mean_got = std::get<1> ( outputs_tpu ).to ( torch::kFloat ).to ( torch::Device ( "cpu" ) );
  auto save_invstd_got = std::get<2> ( outputs_tpu ).to ( torch::kFloat ).to ( torch::Device ( "cpu" ) );
  ...
  if ( track_running_status )
  {
    auto running_mean_got = running_mean_tpu.to ( torch::kFloat ).to ( torch::Device ( "cpu" ) );
    ...
    auto running_var_got = running_var_tpu.to ( torch::kFloat ).to ( torch::Device ( "cpu" ) );

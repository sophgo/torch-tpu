                  ...
                  data_type_t dtype)
{
  ...
  local_addr_t work1_addr = 8 * bank_size;
  local_addr_t work2_addr = 10 * bank_size;
  local_addr_t coeff_addr = 12 * bank_size;
  //nedd to load coeff
...
void tpu_kernel_api_exp(const void *args)
{
  sg_api_exp_t *api = (sg_api_exp_t *)args;
...
void tpu_kernel_api_exp_multi_core(const void *args)
{
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_exp_multi_core);
...
data_type_t dtype )
{
  ...
  int tensor_num=2+2+2+2+1+1+1; // 2 inputs, 2 outputs, 2 work0, 2 work1, 1 exp coeff, 1 log coeff, 1 exp table
  ...
  local_addr_t work1_local_addrs[2] = {4 * tensor_size, 5 * tensor_size};
  local_addr_t work2_local_addrs[2] = {6 * tensor_size, 7 * tensor_size};
  local_addr_t exp_coeff_local_addrs = 8 * tensor_size;
  local_addr_t log_coeff_local_addrs = 9 * tensor_size;
  local_addr_t exp_table_local_addrs = 10 * tensor_size;
  ...
  while ( todo != 0 )
  {
    ...
    if(dtype != DT_FP32) {
        tpu_bdc_cast(input_local_addrs[index], input_local_addrs[index], &shape, NULL, NULL, DT_FP32, dtype, RM_HALF_TO_EVEN);
    }
    tpu_bdc_fp32_pow_C(output_local_addrs[index], input_local_addrs[index], work1_local_addrs[index], work2_local_addrs[index],
                       exp_coeff_local_addrs, log_coeff_local_addrs, exp_table_local_addrs, scalar, &shape);

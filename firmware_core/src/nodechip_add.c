...
void nodechip_add(global_addr_t input_global_addr,
  ...
  int tensor_num = 2 + 2 + 2; // 4 + 4 inputs, 4 outputs
  ...
  while (cur_idx[2] < length) {
    ...
    if (stage_idx > 0 && draning_idx < 2) {
      ...
      if (dtype == DT_FP32 || dtype == DT_FP16 || dtype == DT_BFP16) {
        tpu_bdc_fp_mul_C(other_local_addr[(stage_idx - 1) & 0x1],
                         other_local_addr[(stage_idx - 1) & 0x1], value,
                         &cur_shape, NULL, NULL, dtype);
        ...
        tpu_bdc_int_mul_C(other_local_addr[(stage_idx - 1) & 0x1],
                          other_local_addr[(stage_idx - 1) & 0x1], value,
                          &cur_shape, NULL, NULL, dtype, dtype, dtype, 0,
                          NO_USE, false);
        tpu_bdc_int_add(out_local_addr[(stage_idx - 1) & 0x1],
                        in_local_addr[(stage_idx - 1) & 0x1],
                        other_local_addr[(stage_idx - 1) & 0x1], &cur_shape,
                        NULL, NULL, NULL, dtype, dtype, dtype, 0, NO_USE,
                        false);
      }
    }
...
void tpu_kernel_api_add(const void *args) {
  ...
  tpu_initialize();
  nodechip_add(api->input_global_addr, api->other_global_addr,
               api->output_global_addr, value, length, (data_type_t)api->dtype);
  tpu_poll();
}
...
void nodechip_bcast_add(global_addr_t input_global_addr,
                        global_addr_t other_global_addr,
  ...
  while (true) {
    int size = tpu_aligned_feature_size(hmax, output_shape->w, dtype) *
               c_per_npu * nmax;
    output_addr = 0;
  ...
  while (ctodo > 0) {
    ...
    while (ntodo > 0) {
      ...
      while (htodo > 0) {
        ...
        if (dtype == DT_FP32 || dtype == DT_FP16 || dtype == DT_BFP16) {
          tpu_bdc_fp_mul_C(other_addr, other_addr, value, &slice_shape, NULL,
                           NULL, dtype);
          tpu_bdc_fp_add(output_addr, input_addr, other_addr, &slice_shape,
                         NULL, NULL, NULL, dtype);
        } else {
          tpu_bdc_int_mul_C(other_addr, other_addr, value, &slice_shape, NULL,
                            NULL, dtype, dtype, dtype, 0, NO_USE, false);
          tpu_bdc_int_add(output_addr, input_addr, other_addr, &slice_shape,
                          NULL, NULL, NULL, dtype, dtype, dtype, 0, NO_USE,
                          false);
        }
...
void tpu_kernel_api_bcast_add(const void *args) {
  ...
  tpu_initialize();
  nodechip_bcast_add(api->input_global_addr, api->other_global_addr,
                     api->output_global_addr, &input_shape, &other_shape,
                     &output_shape, &input_global_stride, &other_global_stride,
                     &output_global_stride, value, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_bcast_add);

// #ifdef FIRMWARE_BACKEND_2260
void tpu_kernel_api_bcast_add_multi_core(const void *args) {
  sg_api_bcast_add_t *api = (sg_api_bcast_add_t *)args;
  TPUKERNEL_ASSERT(api->dtype == DT_INT8 || api->dtype == DT_UINT8 ||
                   api->dtype == DT_INT16 || api->dtype == DT_UINT16 ||
                   api->dtype == DT_INT32 || api->dtype == DT_UINT32 ||
  ...
  if (core_idx < allocate_core) {
    nodechip_bcast_add(
        ...
        &input_shape, &other_shape, &output_shape, &input_stride, &other_stride,
        &output_stride, value, (data_type_t)api->dtype);
  }
  tpu_poll();
}

TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_bcast_add_multi_core);

void nodechip_add_scalar(global_addr_t input_global_addr,
                         global_addr_t output_global_addr, scalar_t value,
                         unsigned long long length, data_type_t dtype) {
  if (length == 0) {
    return;
  ...
  while (cur_idx[2] < length) {
    ...
    if (stage_idx > 0 && draning_idx < 2) {
      dim4 cur_shape = {cur_n_dim[1], DIV_UP(cur_m_dim[1], w_dim), 1,
                        w_dim}; // matrix layout shape (n, c, h, w)
      if (dtype == DT_FP32 || dtype == DT_FP16 || dtype == DT_BFP16) {
        tpu_bdc_fp_add_C(out_local_addr[(stage_idx - 1) & 0x1],
                         in_local_addr[(stage_idx - 1) & 0x1], value,
                         &cur_shape, NULL, NULL, dtype);
      } else {
        tpu_bdc_int_add_C(out_local_addr[(stage_idx - 1) & 0x1],
                          in_local_addr[(stage_idx - 1) & 0x1], value,
                          &cur_shape, NULL, NULL, dtype, dtype, dtype, 0,
                          NO_USE, false);
      }
...
void tpu_kernel_api_addc(const void *args) {
  ...
  tpu_initialize();
  nodechip_add_scalar(api->input_global_addr, api->output_global_addr, value,
...
#ifdef FIRMWARE_BACKEND_2260
void tpu_kernel_api_addc_multi_core(const void *args) {
  sg_api_addc_t *api = (sg_api_addc_t *)args;
  TPUKERNEL_ASSERT(api->dtype == DT_INT8 || api->dtype == DT_UINT8 ||
                   api->dtype == DT_INT16 || api->dtype == DT_UINT16 ||
                   api->dtype == DT_INT32 || api->dtype == DT_UINT32 ||
  ...
  TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_addc_multi_core);
#endif

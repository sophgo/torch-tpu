...
static inline bool is_local_mem_enough(
    ...
    size[0] = 1 * tpu_aligned_feature_size(1, ohw, dtype);
    ...
    int total_size = ALIGN(size[0], BANK_SIZE) +
                     ALIGN(size[1], BANK_SIZE) * 2 * (stride_less_kernel ? 3 : 2) +
...
static void maxpool_data_split(
    ...
    bool valid = is_local_mem_enough(size, c, ih * iw, oh * ow, stride_less_kernel, dtype);
    if (valid) {
        index_size = ALIGN(size[0], BANK_SIZE);
        io_size = ALIGN(size[1], BANK_SIZE) * 2 * (stride_less_kernel ? 3 : 2) +
                  ALIGN(size[2], BANK_SIZE) * 2 * 2;
    ...
    */
    // split c
    if (c > NPU_NUM) {
        bool valid = is_local_mem_enough(size, NPU_NUM, ih * iw, oh * ow, stride_less_kernel, dtype);
        if (valid) {
            index_size = ALIGN(size[0], BANK_SIZE);
            io_size = ALIGN(size[1], BANK_SIZE) * 2 * (stride_less_kernel ? 3 : 2) +
                      ALIGN(size[2], BANK_SIZE) * 2 * 2;
    ...
    p_secs->csecs = DIV_UP(c, cslice);
    int hwsecs = 1 + (size[1] * 2 * (stride_less_kernel ? 3 : 2) + size[2] * 2 * 2) / (LOCAL_MEM_SIZE - ALIGN(size[0], BANK_SIZE));
...
void nodechip_maxpool_backward(
    ...
    local_addr_t indices_addr = 0;
    if (stride_h < kh || stride_w < kw) {
        indices_addr = ALIGN(grad_in_pong + osize, BANK_SIZE);
        TPUKERNEL_ASSERT(indices_addr + isize <= (unsigned int)LOCAL_MEM_SIZE);
    }
    ...
    TPUKERNEL_DBG("indices local addr = 0x%5x, bank id = %d\n", indices_addr, indices_addr / BANK_SIZE);
        ...
        for (int cidx = 0; cidx < secs_info.csecs; cidx++) {
            ...
            for (int ohidx = 0; ohidx < secs_info.ohsecs; ohidx++) {
                ...
                for (int owidx = 0; owidx < secs_info.owsecs; owidx++) {
                    ...
                    // stride < kernel use loop for kh*kw
                    if (stride_h == kh && stride_w == kw) {
                        ...
                        tpu_bdc_arithmetic_sequence_bcast(
                            index_addr,
                            1,
                            1,//oslice_shape.h * oslice_shape.w - 1,
                            1,//-1,
                            oslice_shape.h * oslice_shape.w);
                        ...
                        scalar_t one = {.f32 = 1.0f};
                        dim4 per_stride;
                        tpu_compact_stride(&per_stride, 0, &oslice_shape);
                        per_stride.h = oslice_shape.w * stride_h;
                        ...
                        for (int index = 0; index < kh * kw; index++) {
                            //if (index < ((oslice_shape.w + slice_pad.left + slice_pad.right) * slice_pad.top + slice_pad.left) continue;
                            local_addr_t index_offset = ((index / kw) * oslice_shape.w + index % kw) * tpu_data_type_size(dtype);
                            tpu_bdc_equal(
                                ping ? grad_in_ping : grad_in_pong,

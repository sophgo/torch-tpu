...
void nodechip_fmaxc(global_addr_t input_global_addr, scalar_t value,
  ...
  while (todo != 0) {
    ...
    if ((dtype == DT_FP32 && ((value.u32 & 0x7f800000) == 0x7f800000) &&
        ...
        (dtype == DT_FP16 && ((value.u16 & 0x7c00) == 0x7c00) &&
         ((value.u16 & 0x7fff) != 0x7c00)) ||
        (dtype == DT_BFP16 && ((value.u16 & 0x7f80) == 0x7f80) &&
         ((value.u16 & 0x7fff) != 0x7f80))) {
      tpu_bdc_cpy(output_local_addrs[index], input_local_addrs[index], &shape,
                  NULL, NULL, dtype);
    } else {
...
void tpu_kernel_api_fmaxc_multi_core(const void *args) {
  ...
  if (core_idx * length_slice < length) {
    nodechip_fmaxc(api->input_global_addr + (length_slice * core_idx) *
...
void nodechip_fmax(global_addr_t input_global_addr,
...
void tpu_kernel_api_fmax_multi_core(const void *args) {
  ...
  if (core_idx * length_slice < length) {
    nodechip_fmax(api->input_global_addr + (length_slice * core_idx) *
                                               tpu_data_type_size(api->dtype),
...
void nodechip_fmax_bcast(global_addr_t input_global_addr,
                         global_addr_t other_global_addr,
...
void tpu_kernel_api_fmax_bcast(const void *args) {
  ...
  tpu_initialize();
  nodechip_fmax_bcast(api->input_global_addr, api->other_global_addr,
...
void tpu_kernel_api_fmax_bcast_multi_core(const void *args) {
  ...
  if (core_idx * length_slice < length) {
    nodechip_fmax_bcast(api->input_global_addr + input_offset,

...
// void nodechip_repeat_w(global_addr_t input_global_addr, global_addr_t output_global_addr,
//                        int cur_dim, int repeat_times, int dtype) {
//     int dtype_size = tpu_data_type_size(dtype);
//     for(int i = 0; i < repeat_times; ++i) {
//         dim4 cur_shape = {.n=1, .c=1, .h=1, .w=cur_dim};
//         tpu_gdma_cpy_S2S(output_global_addr, input_global_addr, &cur_shape, NULL, NULL, dtype);
//         output_global_addr += cur_dim * dtype_size;
//     }
//     input_global_addr += cur_dim * dtype_size;
// }
void nodechip_repeat(global_addr_t input_global_addr, global_addr_t output_global_addr,
                     int *shape, int *repeat_times, int dim, int repeat_dim, data_type_t dtype) {
        ...
        for(int i = 0; i < repeat_times[repeat_dim - 2]; ++i) {  // 重复h维度
            ...
            for(int h = 0; h < shape[dim - 2]; ++h) {
                for(int k = 0; k < repeat_times[repeat_dim - 1]; ++k) {  // 先重复w维度
                    dim4 cur_shape = {.n=1, .c=1, .h=1, .w=cur_dim};
                    tpu_gdma_cpy_S2S(dst_global_addr, src_global_addr, &cur_shape, NULL, NULL, dtype);
    ...
    if(dim >= 3) {  // 基于上面的代码，c维度的第一个channel已经重复完毕
        for(int c = 1; c < shape[dim - 3]; ++c) { // 所以从c=1开始，与上面对h、w维度的操作一致
            for(int i = 0; i < repeat_times[repeat_dim - 2]; ++i) {
                src_global_addr = input_global_addr + shape[dim - 1] * shape[dim - 2] * dtype_size * c;
                for(int h = 0; h < shape[dim - 2]; ++h) {
        ...
        cur_dim = shape[dim - 1] * repeat_times[repeat_dim - 1]
    ...
    if(dim >= 4) {
        for(int n = 1; n < shape[dim - 4]; ++n) { // 基于上面的代码，n维度的第一个batch已经处理完毕，故从n=1开始
            cur_dim = shape[dim - 1];
            src_global_addr = input_global_addr + n * shape[dim - 1] * shape[dim - 2] * shape[dim - 3] * dtype_size;
            ...
            src_global_addr = output_global_addr + n * shape[dim - 1] * repeat_times[repeat_dim - 1]
                                                     * shape[dim - 2] * repeat_times[repeat_dim - 2]
                                                     * shape[dim - 3] * repeat_times[repeat_dim - 3] * dtype_size;;
            cur_dim = shape[dim - 1] * repeat_times[repeat_dim - 1]
                    * shape[dim - 2] * repeat_times[repeat_dim - 2]

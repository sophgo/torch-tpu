...
/*
 * output = cast(input, Towards Zero)
 */
...
data_type_t dtype) {
    ...
    while(todo != 0) {
        ...
        tpu_bdc_fp_round(output_local_addrs[index],
...
void tpu_kernel_api_trunc(const void* args) {
    sg_api_trunc_t *api = (sg_api_trunc_t*)args;
    ...
    tpu_initialize();
    nodechip_trunc(api->input_global_addr,
                   api->output_global_addr,
                   length,
                   api->dtype);
    tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_trunc);

void tpu_kernel_api_trunc_multi_core(const void* args) {
    sg_api_trunc_t *api = (sg_api_trunc_t*)args;
    TPUKERNEL_ASSERT(api->dtype == DT_FP32 || api->dtype == DT_FP16 || api->dtype == DT_BFP16);
    ...
    nodechip_trunc(api->input_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                   api->output_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                   cur_length_slice,
                   api->dtype);
    tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_trunc_multi_core);
 ...
 */
void nodechip_arcsinh(global_addr_t out_global_addr,
                      ...
                      data_type_t dtype)
{
  ...
  while (cur_idx[2] < length)
  {
    ...
    if (stage_idx > 0 && draning_idx < 2)
    {
      ...
      tpu_bdc_fp32_arcsinh(out_local_addr[(stage_idx - 1) & 0x1],
...
void tpu_kernel_api_arcsinh(const void *args)
{
  ...
  tpu_initialize();
  nodechip_arcsinh(api->output_global_addr, api->input_global_addr, length, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_arcsinh);

void tpu_kernel_api_arcsinh_multi_core(const void *args)
{
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_arcsinh_multi_core);
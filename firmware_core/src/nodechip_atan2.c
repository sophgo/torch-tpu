...
void tpu_kernel_api_atan2_c_multi_core(const void *args) {
    ...
    nodechip_atan2_c(api->input_global_addr+ (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                        api->scalar,
                         api->output_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                         cur_length_slice,
                         (data_type_t)api->dtype);
...
void tpu_kernel_api_atan2_multi_core(const void *args) {
    ...
    nodechip_atan2(api->input_global_addr+ (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                        api->other_global_addr+ (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                         api->output_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->dtype),
                         cur_length_slice,
                         (data_type_t)api->dtype);
...
data_type_t   dtype) {
    ...
    local_addr_t work0_addr, work1_addr, work2_addr,work3_addr,exp_coeff_addr,log_coeff_addr,exp_table_addr,arcsin_coeff_addr;
    while(true) {
        output_addr = 0;
    ...
    while(ctodo > 0) {
        ...
        while(ntodo > 0) {
            ...
            while(htodo > 0) {
                ...
                tpu_bdc_fp_add_C(work2_addr,output_addr, ADD_C, &shape, NULL, NULL, dtype); 
...
void tpu_kernel_api_atan2_bcast(const void *args) {
    ...
    tpu_initialize();
    nodechip_atan2_bcast(api->input_global_addr,
                         api->other_global_addr,
                         api->output_global_addr,
                         &input_shape,
                               &other_shape,

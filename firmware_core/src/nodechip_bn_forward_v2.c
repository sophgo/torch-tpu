...
data_type_t   dtype )
{
  ...
  while ( true )
  {
    ...
    if ( dtype == DT_FP16 )
    {
      ...
      int GDMAISize = tpu_aligned_feature_size ( HMax, W, DT_FP16 ) * DIV_UP ( CMax, NPU_NUM ) * NMax;
      GDMAOAddr = GDMAIAddr + GDMAISize;
      int GDMAOSize = GDMAISize;
      GDMAAddr = GDMAOAddr + GDMAOSize;
      int GDMASize = tpu_aligned_feature_size ( 1,  1, DT_FP16 ) * DIV_UP ( CMax, NPU_NUM );
  ...
  while ( CTodo > 0 )
  {
    ...
    while ( NTodo > 0 )
    {
      ...
      while ( HTodo > 0 )
      {
        ...
        if ( Split == true )
        {
          ...
          if ( dtype == DT_FP16 )
          {
            tpu_bdc_cast ( IOAddr[IOIndex], GDMAIOAddr[IOIndex], &Shape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
          }
        }
        else
        {
          ...
          if ( dtype == DT_FP16 )
          {
            tpu_bdc_cast ( IAddr, GDMAIAddr, &Shape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
          }
        }
        /* P = REDUCE_SUM ( I, [ 3 ] ) */
    ...
    if ( dtype == DT_FP16 )
    {
      tpu_bdc_cast ( GDMAAddr, MAddr, &CShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
      tpu_gdma_cpy_L2S ( batch_mean_global_addr + CDone * DataSize, GDMAAddr, &CShape, NULL, NULL, dtype );
    }
    else
    {
    ...
    if ( running_mean_defined )
    {
      ...
      if ( dtype == DT_FP16 )
      {
        ...
        tpu_bdc_cast ( TAddr, GDMAAddr, &CShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
      }
      else
      {
      ...
      if ( dtype == DT_FP16 )
      {
        tpu_bdc_cast ( GDMAAddr, TAddr, &CShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
        tpu_gdma_cpy_L2S ( updated_mean_global_addr + CDone * DataSize, GDMAAddr, &CShape, NULL, NULL, dtype );
      }
      else
      {
    ...
    if ( running_var_defined )
    {
      ...
      if ( dtype == DT_FP16 )
      {
        ...
        tpu_bdc_cast ( PAddr, GDMAAddr, &CShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
      }
      else
      {
      ...
      if ( dtype == DT_FP16 )
      {
        tpu_bdc_cast ( GDMAAddr, SAddr, &CShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
        tpu_gdma_cpy_L2S ( updated_var_global_addr + CDone * DataSize, GDMAAddr, &CShape, NULL, NULL, dtype );
      }
      else
      {
    ...
    if ( dtype == DT_FP16 )
    {
      tpu_bdc_cast ( GDMAAddr, VAddr, &CShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
      tpu_gdma_cpy_L2S ( batch_invstd_global_addr + CDone * DataSize, GDMAAddr, &CShape, NULL, NULL, dtype );
    }
    else
    {
    ...
    if ( bias_defined )
    {
      ...
      if ( dtype == DT_FP16 )
      {
        ...
        tpu_bdc_cast ( SAddr, GDMAAddr, &CShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
      }
      else
      {
    ...
    while ( NTodo > 0 )
    {
      ...
      while ( HTodo > 0 )
      {
        ...
        if ( Split == true )
        {
          ...
          if ( dtype == DT_FP16 )
          {
            tpu_bdc_cast ( GDMAIOAddr[IOIndex], IOAddr[IOIndex], &Shape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
          }
        }
        else
        {
          ...
          if ( dtype == DT_FP16 )
          {
            tpu_bdc_cast ( GDMAOAddr, OAddr, &Shape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
          }
        }
...
void tpu_kernel_api_batchnorm_forward_v2 ( const void * args )
{
  ...
  api->momentum,
  api->eps,
  api->dtype == SG_DTYPE_FP32 ? DT_FP32 : DT_FP16 );
  tpu_poll();
}

...
data_type_t   dtype )
{
  ...
  const dim4 CShape = { .n = 1, .c = C, .h = 1, .w = 1 };
  dim4 GlobalStride;
  ...
  while ( true )
  {
    Split = NMax != N || HMax != H ;
    IAddr = 0;
    int ISize = tpu_aligned_feature_size ( HMax, W, dtype ) * CPerNPU * NMax;
    OAddr = IAddr + ISize;
    int OSize = ISize;
    PAddr = OAddr + OSize;
    int PSize = tpu_aligned_feature_size ( HMax, 1, dtype ) * CPerNPU * NMax;
    TAddr = PAddr + PSize;
    int TSize = tpu_aligned_feature_size ( 1, 1, dtype ) * CPerNPU * NMax;
    SAddr = TAddr + TSize;
    int SSize = tpu_aligned_feature_size ( 1, NMax, dtype ) * CPerNPU;
    MAddr = SAddr + SSize;
    int MSize = tpu_aligned_feature_size ( 1,  1, dtype ) * CPerNPU;
    VAddr = MAddr + MSize;
    int VSize = MSize;
  ...
  while ( NTodo > 0 )
  {
    ...
    while ( HTodo > 0 )
    {
      ...
      global_addr_t IGAddr = input_global_addr + ( NDone * GlobalStride.n + HDone * GlobalStride.h ) * DataSize;
      if ( Split == true )
      {
  ...
  tpu_gdma_cpy_L2S ( batch_mean_global_addr, MAddr, &CShape, NULL, NULL, dtype );
  /* Move running_mean from global to local memory */
  tpu_gdma_cpy_S2L ( TAddr, running_mean_global_addr, &CShape, NULL, NULL, dtype );
  /* S = M * Momentum */
  ...
  tpu_gdma_cpy_L2S ( updated_mean_global_addr, TAddr, &CShape, NULL, NULL, dtype );
  //////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////
  /* Compute Var */
  IOIndex = 0;
  ...
  tpu_gdma_cpy_S2L ( PAddr, running_var_global_addr, &CShape, NULL, NULL, dtype );
  /* T = V / ( N * H * W - 1 ) */
  ...
  tpu_gdma_cpy_L2S ( updated_var_global_addr, SAddr, &CShape, NULL, NULL, dtype );
  /* T = V / ( N * H * W ) */
  ...
  tpu_gdma_cpy_L2S ( batch_invstd_global_addr, VAddr, &CShape, NULL, NULL, dtype );
  /* Move weight from global to local */
  tpu_gdma_cpy_S2L ( TAddr, weight_global_addr, &CShape, NULL, NULL, dtype );
  /* Move bias from global to local */
  tpu_gdma_cpy_S2L ( SAddr, bias_global_addr, &CShape, NULL, NULL, dtype );
  //////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////
  /* Compute Output */
  IOIndex = 0;
  ...
  while ( NTodo > 0 )
  {
    ...
    while ( HTodo > 0 )
    {
      ...
      if ( Split == true )
      {
        ...
        if ( NDone > 0 || HDone > 0 )
        {
          global_addr_t OGAddr = output_global_addr + ( NDoneLast * GlobalStride.n + HDoneLast * GlobalStride.h ) * DataSize;
      ...
      {
        global_addr_t OGAddr = output_global_addr + ( NDone * GlobalStride.n + HDone * GlobalStride.h ) * DataSize;

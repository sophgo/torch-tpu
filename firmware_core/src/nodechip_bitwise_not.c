...
data_type_t dtype )
{
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_not(output_local_addrs[index],input_local_addrs[index],&shape,NULL,NULL,dtype);
...
void tpu_kernel_api_bitwise_not ( const void * args )
{
  ...
  tpu_initialize();
  nodechip_bitwise_not ( api->input_global_addr, api->output_global_addr, length, ( data_type_t ) api->dtype );
  tpu_poll();
}

...
data_type_t dtype )
{
  ...
  local_addr_t exp_table_local_addr = 6 * tensor_size + 1 * bank_size;
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_fp32_C_pow(output_local_addrs[index],input_local_addrs[index],work_local_addr[0],work_local_addr[1],
                          exp_coeff_local_addr,exp_table_local_addr,C,&shape);
...
void tpu_kernel_api_exp2 ( const void * args )
{
  sg_api_exp2_t * api = ( sg_api_exp2_t * ) args;
  ...
  tpu_initialize();
  nodechip_exp2 ( api->input_global_addr, api->output_global_addr, length, ( data_type_t ) api->dtype );
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_exp2 );

void tpu_kernel_api_exp2_multi_core ( const void * args )
{
  sg_api_exp2_t * api = ( sg_api_exp2_t * ) args;
  TPUKERNEL_ASSERT ( api->dtype == DT_FP32);
...
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_exp2_multi_core );
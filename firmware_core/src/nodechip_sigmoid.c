...
static inline void nodechip_sigmoid_forward_parallel ( global_addr_t XGlobalAddr, global_addr_t YGlobalAddr, int Len, data_type_t dtype)
{
  ...
  while ( Todo > 0 )
  {
    ...
    tpu_bdc_fp32_sigmoid ( YCastAddrs[Index], XCastAddrs[Index], W0Addr, W1Addr, EXPCoeffAddr, EXPTableAddr, &Shape );
...
void tpu_kernel_api_sigmoid(const void *args) {
  sg_api_sigmoid_t *api = (sg_api_sigmoid_t *) args;
  ...
  nodechip_sigmoid_forward_parallel ( api->input_global_addr, api->output_global_addr, Len, api->dtype );
  }
  else
  {
    tpu_initialize();
...
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_sigmoid);

void tpu_kernel_api_sigmoid_multi_core(const void *args) {
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_sigmoid_multi_core);
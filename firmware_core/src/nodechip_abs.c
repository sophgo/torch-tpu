...
void tpu_kernel_api_abs(const void *args)
{
  sg_api_abs_t *api = (sg_api_abs_t *)args;
  ...
  tpu_initialize();
  nodechip_scale_abs(api->input_global_addr, api->output_global_addr, length, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_abs);

void tpu_kernel_api_abs_multi_core(const void *args)
{
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_abs_multi_core);
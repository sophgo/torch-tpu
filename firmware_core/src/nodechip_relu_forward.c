...
void tpu_kernel_api_relu ( const void * args )
{
  sg_api_relu_t * api = ( sg_api_relu_t * ) args;
  ...
  tpu_initialize();
  scalar_t value = { .u32 = 0 };
  nodechip_maxc ( api->input_global_addr, api->output_global_addr, value, length, ( data_type_t ) api->dtype );
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_relu );

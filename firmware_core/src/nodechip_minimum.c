...
void nodechip_minimumc(global_addr_t input_global_addr, scalar_t value,
  ...
  while (todo != 0) {
    ...
    if (dtype == DT_FP32 && ((value.u32 & 0x7f800000) == 0x7f800000) &&
        ((value.u32 & 0x7fffffff) != 0x7f800000)) {
      tpu_bdc_set_C(output_local_addrs[index], value, &shape, NULL, dtype);
    } else if (dtype == DT_FP16 && ((value.u16 & 0x7c00) == 0x7c00) &&
               ((value.u16 & 0x7fff) != 0x7c00)) {
      tpu_bdc_set_C(output_local_addrs[index], value, &shape, NULL, dtype);
    } else if (dtype == DT_BFP16 && ((value.u16 & 0x7f80) == 0x7f80) &&
               ((value.u16 & 0x7fff) != 0x7f80)) {
      tpu_bdc_set_C(output_local_addrs[index], value, &shape, NULL, dtype);
    } else {
...
int tpu_kernel_api_minimumc(const void *args) {
  ...
  tpu_initialize();
  nodechip_minimumc(api->input_global_addr, value, api->output_global_addr,
...
int tpu_kernel_api_minimumc_multi_core(const void *args) {
  sg_api_minimumc_t *api = (sg_api_minimumc_t *)args;
  TPUKERNEL_ASSERT(api->dtype == DT_FP32 || api->dtype == DT_FP16 ||
                   api->dtype == DT_BFP16 || api->dtype == DT_INT32);
...
void nodechip_minimum(global_addr_t input_global_addr,
  ...
  while (todo != 0) {
    ...
    if (dtype != DT_INT32) {
      ...
      replaceWithNan(input_local_addrs[index], output_local_addrs[index],
                     work_local_addrs[0], work_local_addrs[1],
                     work_local_addrs[2], &shape, NULL, dtype);
      isNan(work_local_addrs[0], other_local_addrs[index], work_local_addrs[1],
            work_local_addrs[2], &shape, NULL, dtype);
      replaceWithNan(output_local_addrs[index], input_local_addrs[index],
                     work_local_addrs[0], work_local_addrs[1],
                     work_local_addrs[2], &shape, NULL, dtype);
    }
...
int tpu_kernel_api_minimum(const void *args) {
  ...
  tpu_initialize();
  nodechip_minimum(api->input_global_addr, api->other_global_addr,
...
#ifdef BACKEND_SG2260
int tpu_kernel_api_minimum_multi_core(const void *args) {
  sg_api_minimum_t *api = (sg_api_minimum_t *)args;
  TPUKERNEL_ASSERT(api->dtype == DT_FP32 || api->dtype == DT_FP16 ||
                   api->dtype == DT_BFP16 || api->dtype == DT_INT32);
...
void nodechip_minimum_bcast(global_addr_t input_global_addr,
                            global_addr_t other_global_addr,
      ...
      while (htodo > 0) {
        ...
        if (dtype != DT_INT32) {
          isNan(work0_addr, input_addr, work1_addr, work2_addr, &shape, NULL,
                dtype);
          replaceWithNan(input_addr, output_addr, work0_addr, work1_addr,
                         work2_addr, &shape, NULL, dtype);
          isNan(work0_addr, other_addr, work1_addr, work2_addr, &shape, NULL,
                dtype);
          replaceWithNan(output_addr, input_addr, work0_addr, work1_addr,
                         work2_addr, &shape, NULL, dtype);
        }
...
int tpu_kernel_api_minimum_bcast(const void *args) {
  ...
  tpu_initialize();
  nodechip_minimum_bcast(api->input_global_addr, api->other_global_addr,
...
#ifdef BACKEND_SG2260
int tpu_kernel_api_minimum_bcast_multi_core(const void *args) {
  sg_api_minimum_bcast_t *api = (sg_api_minimum_bcast_t *)args;
  TPUKERNEL_ASSERT(api->output_dim > 0 && api->output_dim <= 4);
...
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_minimum_bcast_multi_core);
#endif
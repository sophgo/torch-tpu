...
data_type_t dtype )
{
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_fp32_arccos(output_local_addrs[index],input_local_addrs[index],
...
void tpu_kernel_api_acos ( const void * args )
{
  sg_api_acos_t * api = ( sg_api_acos_t * ) args;
  ...
  tpu_initialize();
  nodechip_acos ( api->input_global_addr, api->output_global_addr, length, ( data_type_t ) api->dtype );
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_acos );

void tpu_kernel_api_acos_multi_core ( const void * args )
{
  sg_api_acos_t * api = ( sg_api_acos_t * ) args;
  TPUKERNEL_ASSERT ( api->dtype == DT_FP32);
...
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_acos_multi_core );
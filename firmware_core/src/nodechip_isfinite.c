...
data_type_t dtype )
{
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_and_C(work_local_addr, input_local_addrs[index], C, &shape, NULL, NULL, dtype);
    tpu_bdc_not_equal_C(output_local_addrs[index], work_local_addr, C, C1, &shape, NULL, NULL, DT_UINT8, dtype);

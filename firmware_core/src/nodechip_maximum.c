...
static void isNan(local_addr_t output_local_addr, local_addr_t input_local_addr,
...
static void replaceWithNan(local_addr_t output_local_addr,
...
void nodechip_maximumc(global_addr_t input_global_addr, scalar_t value,
  ...
  while (todo != 0) {
    ...
    tpu_bdc_max_C(output_local_addrs[index], input_local_addrs[index], value,
                  &shape, NULL, NULL, dtype);
    if (dtype != DT_INT32) {
      isNan(work_local_addrs[0], input_local_addrs[index], work_local_addrs[1],
            work_local_addrs[2], &shape, NULL, DT_UINT8);
...
void tpu_kernel_api_maximumc(const void *args) {
...
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_maximumc_multi_core);

void nodechip_maximum(global_addr_t input_global_addr,
  ...
  while (todo != 0) {
    ...
    tpu_bdc_max(output_local_addrs[index], input_local_addrs[index],
...
void tpu_kernel_api_maximum(const void *args) {
  ...
  tpu_initialize();
  nodechip_maximum(api->input_global_addr, api->other_global_addr,
...
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_maximum_multi_core);

void nodechip_maximum_bcast(global_addr_t input_global_addr,
                            global_addr_t other_global_addr,
  ...
  while (ctodo > 0) {
    ...
    while (ntodo > 0) {
      ...
      while (htodo > 0) {
        ...
        // calculate
        tpu_bdc_max(output_addr, input_addr, other_addr, &shape, NULL, NULL,
                    NULL, dtype);
        if (dtype != DT_INT32) {
...
void tpu_kernel_api_maximum_bcast(const void *args) {
  ...
  tpu_initialize();
  nodechip_maximum_bcast(api->input_global_addr, api->other_global_addr,
...
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_maximum_bcast_multi_core);
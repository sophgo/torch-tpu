 ...
 */
void nodechip_arctanh(global_addr_t out_global_addr,
                      ...
                      data_type_t dtype)
{
  ...
  tpu_bdc_load_fp32_log_coeff(coeff_addr);
  ...
  while (cur_idx[2] < length)
  {
    ...
    if (stage_idx > 0 && draning_idx < 2)
    {
      ...
      tpu_bdc_fp32_arctanh(out_local_addr[(stage_idx - 1) & 0x1],
...
void tpu_kernel_api_arctanh(const void *args)
{
  ...
  tpu_initialize();
  nodechip_arctanh(api->output_global_addr, api->input_global_addr, length, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_arctanh);

void tpu_kernel_api_arctanh_multi_core(const void *args)
{
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_arctanh_multi_core);
...
data_type_t dtype )
{
  ...
  int tensor_num=2+2; // 2 inputs, 2 outputs, 0 buffer
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_fp_round(output_local_addrs[index],input_local_addrs[index],&shape,NULL,NULL,dtype,RM_HALF_TO_EVEN);
...
void tpu_kernel_api_round ( const void * args )
{
  sg_api_round_t * api = ( sg_api_round_t * ) args;
  ...
  tpu_initialize();
  nodechip_round ( api->input_global_addr, api->output_global_addr, length, ( data_type_t ) api->dtype );
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_round );

void tpu_kernel_api_round_multi_core ( const void * args )
{
  sg_api_round_t * api = ( sg_api_round_t * ) args;
  TPUKERNEL_ASSERT ( api->dtype == DT_FP32);
...
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_round_multi_core );
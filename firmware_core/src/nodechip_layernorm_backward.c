    ...
    */
    ...
    while ( NTodo > 0 )
    {
        ...
        while ( CTodo > 0 )
        {
            ...
            if( dtype == DT_FP16 )
            {
                tpu_gdma_cpy_S2L ( grad_outputAddr_fp16, grad_outputGAddr, &Shape, NULL, &GlobalStride, DT_FP16 );
                tpu_gdma_cpy_S2L ( inputAddr_fp16, inputGAddr, &Shape, NULL, &GlobalStride, DT_FP16 );
                tpu_gdma_cpy_S2L ( meanAddr_fp16, meanGAddr, &ParamShape, NULL, NULL, DT_FP16 );
                tpu_gdma_cpy_S2L ( invstdAddr_fp16, invstdGAddr, &ParamShape, NULL, NULL, DT_FP16 );
                if( !NDone && !CDone )
                {
                    tpu_gdma_cpy_S2L ( weightAddr_fp16, weightGAddr, &WeightShape, NULL, NULL, DT_FP16 );
                    tpu_bdc_npu_bcast( weightAddr_fp16, weightAddr_fp16, &WeightBCastShape, DT_FP16);
                }
                tpu_bdc_cast ( grad_outputAddr, grad_outputAddr_fp16, &Shape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
                tpu_bdc_cast ( inputAddr, inputAddr_fp16, &Shape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
                tpu_bdc_cast ( meanAddr, meanAddr_fp16, &ParamShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
                tpu_bdc_cast ( invstdAddr, invstdAddr_fp16, &ParamShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
                tpu_bdc_cast ( weightAddr, weightAddr_fp16, &WeightBCastShape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN );
            }
    ...
    if ( dtype == DT_FP16 )
    {
        tpu_bdc_cast ( grad_biasAddr, grad_biasAddr, &WeightShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
        tpu_bdc_cast ( grad_weightAddr, grad_weightAddr, &WeightShape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN );
    }
    if ( grad_bias_enable ) tpu_gdma_cpy_L2S ( grad_bias_global_addr, grad_biasAddr, &WeightShape, NULL, NULL, dtype );
    if ( grad_weight_enable ) tpu_gdma_cpy_L2S ( grad_weight_global_addr, grad_weightAddr, &WeightShape, NULL, NULL, dtype );
}

...
get_const_binary_fp32_div_func(bool inversed) {
  ...
  if (inversed)
    func = tpu_bdc_fp32_C_div;
  else if (!inversed)
    func = tpu_bdc_fp32_div_C;
  return func;
}
    ...
    if (stage_idx > 0 && draning_idx < 2) {
      ...
      if (binary_type == BINARY_DIV) {
        ...
        if (binary_type == BINARY_ADD || binary_type == BINARY_SUB) {
          ...
          func_const(other_local_addr[(stage_idx - 1) & 0x1],
                     other_local_addr[(stage_idx - 1) & 0x1], value, &cur_shape,
                     NULL, NULL, dtype);
        }
        ...
        func(out_local_addr[(stage_idx - 1) & 0x1],
             in_local_addr[(stage_idx - 1) & 0x1],
             other_local_addr[(stage_idx - 1) & 0x1], &cur_shape, NULL, NULL,
             NULL, dtype);
        ...
        if (binary_type == BINARY_ADD || binary_type == BINARY_SUB) {
          ...
          func_const(other_local_addr[(stage_idx - 1) & 0x1],
                     other_local_addr[(stage_idx - 1) & 0x1], value, &cur_shape,
                     NULL, NULL, dtype, dtype, dtype, 0, NO_USE, false);
        }
...
void tpu_kernel_api_binary_multi_core(const void *args) {
  ...
  if (core_idx < length_secs) {
    nodechip_binary(api->input_global_addr + (length_slice * core_idx) * dsize,
                    api->other_global_addr + (length_slice * core_idx) * dsize,
                    api->output_global_addr + (length_slice * core_idx) * dsize,
                    value, cur_length_slice, (data_type_t)api->dtype,
...
void tpu_kernel_api_binary_c(const void *args) {
  ...
  tpu_initialize();
  nodechip_binary_scalar(api->input_global_addr, api->output_global_addr, value,

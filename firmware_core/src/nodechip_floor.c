...
data_type_t dtype )
{
  ...
  while ( todo != 0 )
  {
    ...
    tpu_bdc_fp_floor(output_local_addrs[index],input_local_addrs[index],&shape,NULL,NULL,dtype);
...
void tpu_kernel_api_floor ( const void * args )
{
  sg_api_floor_t * api = ( sg_api_floor_t * ) args;
  ...
  tpu_initialize();
  nodechip_floor ( api->input_global_addr, api->output_global_addr, length, ( data_type_t ) api->dtype );
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_floor );

void tpu_kernel_api_floor_multi_core ( const void * args )
{
  sg_api_floor_t * api = ( sg_api_floor_t * ) args;
  TPUKERNEL_ASSERT ( api->dtype == DT_FP32);
...
TPUKERNEL_FUNC_REGISTER ( tpu_kernel_api_floor_multi_core );
 ...
 */
void nodechip_expm1_single_core(global_addr_t out_global_addr,
  ...
  local_addr_t out_local_addr[2] = {2 * tensor_bsize_pnpu,
                                    3 * tensor_bsize_pnpu};
  local_addr_t work1_addr = 4 * tensor_bsize_pnpu;
  local_addr_t work2_addr = 6 * tensor_bsize_pnpu;
  local_addr_t coeff_addr = 8 * tensor_bsize_pnpu;
  // nedd to load coeff
  tpu_bdc_load_fp_exp_coeff(coeff_addr, dtype);

  scalar_t C = {.f32 = 1.f};
  if (dtype != DT_FP32)
    C = tpu_fp_cast(C, dtype, DT_FP32, RM_HALF_TO_EVEN);
  ...
  while (cur_idx[2] < length) {
    ...
    if (stage_idx > 0 && draning_idx < 2) {
      ...
      tpu_bdc_fp_exp(out_local_addr[(stage_idx - 1) & 0x1],
                     in_local_addr[(stage_idx - 1) & 0x1], work1_addr,
                     work2_addr, coeff_addr, &cur_shape, dtype);
      tpu_bdc_fp_sub_C(out_local_addr[(stage_idx - 1) & 0x1],
                       out_local_addr[(stage_idx - 1) & 0x1], C, &cur_shape,
                       NULL, NULL, dtype);
    }
...
void tpu_kernel_api_expm1(const void *args) {
  sg_api_expm1_t *api = (sg_api_expm1_t *)args;
  ...
  tpu_initialize();
  nodechip_expm1_single_core(api->output_global_addr, api->input_global_addr,
                             length, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_expm1);

void tpu_kernel_api_expm1_multi_core(const void *args) {
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_expm1_multi_core);
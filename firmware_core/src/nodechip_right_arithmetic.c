...
void nodechip_shift_right_arithmetic (
...
data_type_t shift_dtype)
{
  ...
  while (cur_idx[2] < length)
  {
    ...
    {
      tpu_gdma_matrix_L2S(
          output_global_addr + cur_idx[2] * tpu_data_type_size(dst_dtype),
          output_local_addr[stage_idx & 0x1],
          cur_n_dim[2], cur_m_dim[2], w_dim, cur_m_dim[2], dst_dtype);
    }
    ...
    if (stage_idx > 0 && draning_idx < 2)
    {
      ...
      //to ensure shift is positive, maybe need to optimize
      tpu_bdc_abs(other_local_addr[(stage_idx - 1) & 0x1],
                  other_local_addr[(stage_idx - 1) & 0x1],
                  &cur_shape, NULL, NULL, shift_dtype);
      tpu_bdc_neg(other_local_addr[(stage_idx - 1) & 0x1],
                  other_local_addr[(stage_idx - 1) & 0x1],
                  &cur_shape, NULL, NULL, shift_dtype);           
      tpu_bdc_arithmetic_shift(output_local_addr[(stage_idx - 1) & 0x1],
                  input_local_addr[(stage_idx - 1) & 0x1],
                  other_local_addr[(stage_idx - 1) & 0x1],
                  &cur_shape, NULL, NULL, NULL, dst_dtype, src_dtype, shift_dtype, 5);
    }
...
void tpu_kernel_api_shift_right_arithmetic_multi_core(const void *args)
{
  ...
  nodechip_shift_right_arithmetic(
...
data_type_t src_dtype)
{
  ...
  while (cur_idx[2] < length)
  {
    ...
    if (stage_idx > 0 && draning_idx < 2)
    {
      ...
      dim4 cur_shape = {cur_n_dim[1], DIV_UP(cur_m_dim[1], w_dim), 1, w_dim};
      //need to check, when shift is negative, cpu still consider it as positive
      shift = shift < 0 ? shift : -shift;
      tpu_bdc_arithmetic_shift_C(output_local_addr[(stage_idx - 1) & 0x1],
                  input_local_addr[(stage_idx - 1) & 0x1],
                  shift,&cur_shape, NULL, NULL, dst_dtype, src_dtype, 5);
...
void tpu_kernel_api_shift_right_arithmetic_c_multi_core(const void *args)
{
  ...
  if (core_idx == length_secs - 1)
    cur_length_slice = length - length_slice * (length_secs - 1);
  nodechip_shift_right_arithmetic_c(
      api->output_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->src_dtype),
      api->input_global_addr + (length_slice * core_idx) * tpu_data_type_size(api->src_dtype),

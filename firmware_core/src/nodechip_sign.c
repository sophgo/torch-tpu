    ...
    data_type_t dtype)
{
  ...
  while (todo != 0)
  {
    ...
    tpu_bdc_sign(output_local_addrs[index], input_local_addrs[index], &shape, dtype);
    l2s = true;
...
void tpu_kernel_api_sign(const void *args)
{
  sg_api_sign_t *api = (sg_api_sign_t *)args;
  ...
  tpu_initialize();
  nodechip_scale_sign(( global_addr_t)api->input_global_addr, (global_addr_t)api->output_global_addr, length, (data_type_t)api->dtype);
  tpu_poll();
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_sign);

void tpu_kernel_api_sign_multi_core(const void *args)
{
  TPUKERNEL_ASSERT_INFO(false, "not implementated");
}
TPUKERNEL_FUNC_REGISTER(tpu_kernel_api_sign_multi_core);

    ...
    float* coef)
{
  ...
  local_addr_t work0_addr, work2_addr, work3_addr;
  ...
  if (!if_local_layer) {
    ...
    work2_addr = ALIGN(work0_addr + tensor_size, bank_size);
    work3_addr = ALIGN(work2_addr + tensor_size, bank_size);
  }
  local_addr_t exp_coeff_addr = ALIGN(work3_addr + tensor_size, bank_size);
  ...
  local_addr_t in_cast_addr = ALIGN(exp_table_addr + 192 * sizeof(float), ALIGN_BYTES);
  local_addr_t out_cast_addr = ALIGN(in_cast_addr + tensor_size, ALIGN_BYTES);
  ...
  tpu_bdc_cast(in_cast_addr, in_addr, shape, NULL, NULL, DT_FP32, DT_FP16, RM_HALF_TO_EVEN);
  // if(tpu_is_parallel_state()){ tpu_parallel_end(); }
  // tpu_poll();
  // float* work_ptr = (float*)tpu_local_mem_addr(0, work0_addr);
  // UNUSED(work_ptr);
  tpu_bdc_fp32_gelu(
      out_cast_addr,
      in_cast_addr,
      ...
      exp_table_addr,
      shape);
  tpu_bdc_cast(out_addr, out_cast_addr, shape, NULL, NULL, DT_FP16, DT_FP32, RM_HALF_TO_EVEN);
}

 ...
 */
void nodechip_logical_not(global_addr_t out_global_addr,
                  ...
                  data_type_t dtype)
{
  ...
  int tensor_num = 4; // 2 outputs, 2 input1
  ...
  local_addr_t out_local_addr[2] = {8* bank_size, 12 * bank_size};
  ...
  while (cur_idx[2] < length)
  {
    ...
    if (stage_idx > 0 && draning_idx < 2)
    {
      ...
      tpu_bdc_equal_C(out_local_addr[(stage_idx - 1) & 0x1],
                    in_local_addr[(stage_idx - 1) & 0x1],
                    (scalar_t)0.0f, (scalar_t)1.0f,
                    &cur_shape, NULL, NULL, dtype ,dtype);
    }
...
int tpu_kernel_api_logical_not(const void *args)
{
  ...
  tpu_initialize();
  nodechip_logical_not(api->output_global_addr, api->input_global_addr, length, (data_type_t)api->dtype);
  tpu_poll();
  return 0;
}
...
int tpu_kernel_api_logical_not_multi_core(const void *args)
{
  sg_api_logical_not_t * api = ( sg_api_logical_not_t * ) args;
  TPUKERNEL_ASSERT(api->dtype == DT_FP32 || api->dtype == DT_FP16 || api->dtype == DT_BFP16);
  int length = 1;

    ...
    data_type_t dtype)
{
    ...
    int buffer_size = DIV_UP(c,NPU_NUM) * 64;
    // mode 0 : split c
    // mode 1 : split hw first loop
    // mode 2 : split hw second loop
    int total_size = mode==0 ? ALIGN(ALIGN(c_param_size * 3, 64) + buffer_size, BANK_SIZE) +
                               ALIGN(param_size, BANK_SIZE) * 3 :
                     mode==1 ? ALIGN(ALIGN(c_param_size * 5, 64) + buffer_size, BANK_SIZE) +
...
{   
    ...
    int buffer_size = DIV_UP(csecs, NPU_NUM) * 64;
    ...
    local_addr_t pooling_buffer = ALIGN(3 * c_param_size, 64);
    ...
    local_addr_t input_local_addr = ALIGN(buffer_size + pooling_buffer, BANK_SIZE);
    ...
    dim2 pooling_kernel = {pooling_shape.h, pooling_shape.w};
    ...
    data_type_t dtype)
{
    ...
    local_addr_t pooling_buffer = ALIGN(c_param_size * 5, 64);
    //param (n,c,h,w) aligned
    local_addr_t input_local_addr = ALIGN(pooling_buffer + buffer_size, BANK_SIZE);
    local_addr_t grad_output_local_addr = input_local_addr + ALIGN(param_size, BANK_SIZE);

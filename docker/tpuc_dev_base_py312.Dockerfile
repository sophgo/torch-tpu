...
RUN apt-get update -y \
    ...
    && apt-get install -y python${PYTHON_VERSION} python${PYTHON_VERSION}-dev python${PYTHON_VERSION}-venv python3-distutils \
...
RUN apt-get purge -y python3-blinker && \
    pip install --upgrade --force-reinstall blinker==1.7.0

# 更新 alternatives 使 /usr/bin/python 指向 python3.12
RUN update-alternatives --install /usr/bin/python python /usr/bin/python3.12 10 && \
    ...
    update-alternatives --install /usr/bin/pip pip /usr/local/bin/pip3.12 10 && \
    update-alternatives --install /usr/bin/python3-config python3-config /usr/bin/python3.12-config 10
...
RUN pip install pybind11-global==2.11.1 numpy==1.26.0 PyYAML==6.0.1 && \
    rm -rf ~/.cache/pip/*

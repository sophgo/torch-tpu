...
if __name__ == "__main__":
    ...
    parser.add_argument("case", type=str, choices=["bert", "gpt"])
    parser.add_argument("--master_addr", type=str, default="172.28.143.14")
    ...
    LOCAL_RANK=0 # f"$((i % {args.local_size}))" # we treat all local size as 1 to avoid confusion
    ...
    LOCAL_SIZE=1 # args.local_size
    MASTER_ADDR=args.master_addr
    if "," in CMODEL_DIST_ADDRS:
        assert MASTER_ADDR in CMODEL_DIST_ADDRS.split(","), "master addr must be in cmodel dist addrs"
    ...
    flags = f"{DS_DISABLE_TPU=} {CUDA_VISIBLE_DEVICES=}" if args.device == 'cuda' else f"{CMODEL_DIST_ADDRS=}"

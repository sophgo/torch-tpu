...
Tensor & trunc_out_tpu(const Tensor &self, Tensor &out) {
  if(self.dim() > 0) CHECK_TENSOR_IN_DEVICE(self);
  CHECK_TENSOR_IN_DEVICE(out);
  ...
  if(self.dim() == 0) {
    ...
    bm_status_t status = sgdnnTrunc(tpu::TPUGetDeviceHandle(),
                                    tpu::TPUGenerateSgdnnTensor(self),
                                    tpu::TPUGenerateSgdnnTensor(out));
    TORCH_CHECK(status == BM_SUCCESS);

#ifdef TPU_OP_TIMING
    tpu::OpTimer::Instance().AddTime(tpu::TRUNC, timer.ElapsedUS());
#endif
...
Tensor trunc_tpu(const Tensor &self) {
  auto out = empty_like(self);
  return trunc_out_tpu(self, out);
}
TORCH_LIBRARY_IMPL(aten, TPU, m) {

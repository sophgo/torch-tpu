...
Tensor repeat_tpu(const Tensor &self, const IntArrayRef repeats) {
    ...
    if(self.dim() == 0) {
        ...
        tpu::TPUCopyDeviceToDevice(out.data_ptr(), out_cpu.contiguous().data_ptr(), out.nbytes());
    }
...
TIMING_START
    bm_status_t status = sgdnnRepeat(tpu::TPUGetDeviceHandle(), tpu::TPUGenerateSgdnnTensor(self),

...
namespace at
{
    ...
    Tensor &abs_out_tpu(const Tensor &self, Tensor &out)
    {
        if (self.dim() > 0)
        {
  ...
  tpu::TPUCopyHostToDevice ( self.data_ptr(),self.contiguous().data_ptr(), self.nbytes() );
    ...
    Tensor abs_tpu(const Tensor &self)
    {
        auto out = empty(self.sizes(), self.options());
        return abs_out_tpu(self, out);
    }
